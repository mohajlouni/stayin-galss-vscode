...
export default function BookingForm() {
  ...
  const notifyBookingSaved = () => { if (Platform.OS === "android") ToastAndroid.show(language === "ar" ? "تم تسجيل الحجز بنجاح" : "Booking saved successfully", ToastAndroid.SHORT); };
  ...
  const save = async () => {
    ...
    if (flexibleSlot) {
      if (!startTime.trim() || !endTime.trim() || (endDate === startDate && timeToMinutes(endTime) <= timeToMinutes(startTime))) return Alert.alert(language === "ar" ? "أوقات غير صحيحة" : "Invalid times", language === "ar" ? "أدخل وقت وصول ووقت مغادرة صحيحين للفترة المخصصة؛ يجب أن يكون وقت المغادرة بعد وقت الوصول." : "Enter valid arrival and departure times for the custom period; departure must be after arrival.");
