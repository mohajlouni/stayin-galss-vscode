    ...
    <ScreenContainer edges={["top", "bottom", "left", "right"]}>
      <ScrollView style={{ flex: 1, backgroundColor: "transparent" }} contentContainerStyle={[styles.content, { direction: layoutDirection }]} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
        ...
        <Section title={language === "ar" ? "النظام والنسخ الاحتياطي" : "System & backup"} icon="settings" colors={colors} align={align} isRTL={isRTL}>
          ...
          <Pressable onPress={() => void exportBackup()} style={[styles.backup, { flexDirection: row, backgroundColor: colors.primary }]}><MaterialIcons name="upload-file" size={19} color="#FFFFFF" /><Text style={{ color: "#FFFFFF", fontWeight: "800" }}>{language === "ar" ? "تصدير نسخة احتياطية" : "Export backup"}</Text></Pressable>
          <Pressable onPress={async () => { const opened = await openBackupForPreview(); if (opened) router.push("/backup-preview" as never); }} style={[styles.backup, { flexDirection: row, backgroundColor: colors.surfaceMuted }]}><MaterialIcons name="file-download" size={19} color={colors.primary} /><Text style={{ color: colors.primary, fontWeight: "800" }}>{language === "ar" ? "استيراد واستعراض نسخة" : "Import and preview backup"}</Text></Pressable>
        </Section>
