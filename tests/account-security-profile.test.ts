...
const unifiedAuth = source("components/unified-auth-screen.tsx");
    ...
    expect(security).toContain("startOAuthLogin");
    expect(security).toContain("تعيين أو إعادة تعيين كلمة المرور");
    expect(security).toContain("لا تُخزن كلمة المرور داخل التطبيق");
    expect(security).toContain("<CompactScreenHeader plain");
    expect(security).toContain("setConfirmation(\"password\")");
