...
type FloatDraft = { id?: string; entity: "staff" | "guard"; label: string; uid: string; phone: string; hasCash: boolean; hasOther: boolean; otherNote: string; ceiling: string; isActive: boolean; isDefault: boolean };
const isChannelDataComplete = (channelType: "cliq" | "bank" | "cash", data: { aliasOrPhone: string; bankOrWallet: string; bankName: string; iban: string }): boolean => {
  if (channelType === "cash") return true;
  if (channelType === "cliq") return data.aliasOrPhone.trim().length > 0 && data.bankOrWallet.trim().length > 0;
  return data.bankName.trim().length > 0 && data.iban.trim().length >= 15;
};

const emptyOwnerRow = (kind: OwnerTreasuryKind): OwnerRow => ({ key: createId(), kind, label: "", detail: "", provider: "", holderName: "", iban: "", isDefault: false, whatsApp: false, isActive: true });

const emptyFloatDraft = (entity: "staff" | "guard"): FloatDraft => ({ entity, label: "", uid: "", phone: "", hasCash: true, hasOther: false, otherNote: "", ceiling: "", isActive: true, isDefault: false });
...
export default function PaymentMethodsScreen() {
  ...
  const openOwnerEdit = (account: OwnerTreasuryAccount) => {
    ...
    setOpenChannels({ cliq: account.kind === "cliq" || ownerAccounts.some((other) => other.id !== account.id && other.kind === "cliq"), bank: account.kind === "bank" || ownerAccounts.some((other) => other.id !== account.id && other.kind === "bank"), cash: account.kind === "vault" || ownerAccounts.some((other) => other.id !== account.id && other.kind === "vault"), other: false });
    setUnifiedOpen(true);
  };
  ...
  const openFloatEdit = (account: StaffFloatAccount) => {
    ...
    setFloatDraft({ id: account.id, entity, label: account.label, uid: selectableMembers.find((member) => member.userId === account.memberUserId)?.userCode ?? "", phone: account.contactPhone ?? "", hasCash: account.isActive !== false, hasOther: false, otherNote: "", ceiling: account.maxFloatLimit !== undefined ? String(account.maxFloatLimit) : "", isActive: account.isActive !== false, isDefault: account.isDefault === true });
  ...
  const saveOwnerEditor = async () => {
    ...
    for (const item of rows) {
      let label: string;
      if (item.kind === "cliq") {
        if (!item.detail.trim() || !item.provider.trim()) { Alert.alert(language === "ar" ? "بيانات CliQ مطلوبة" : "CliQ details required", language === "ar" ? "أدخل الاسم المستعار أو رقم الموبايل واسم البنك أو المحفظة." : "Enter the alias/mobile number and the bank or wallet name."); return; }
        label = item.holderName.trim() || item.detail.trim();
      } else if (item.kind === "bank") {
        if (!item.provider.trim() || !item.iban.trim() || item.iban.trim().length < 15) { Alert.alert(language === "ar" ? "بيانات الحساب البنكي غير مكتملة" : "Incomplete bank account details", language === "ar" ? "أدخل اسم البنك ورقم الآيبان الدولي (15 خانة على الأقل)." : "Enter the bank name and an international IBAN (15+ characters)."); return; }
    ...
    let merged = [...ownerAccounts.filter((account) => !next.some((item) => item.id === account.id)), ...next];
    if (!merged.some((account) => account.isDefault === true)) {
      const fallback = merged.find((account) => account.kind === "vault") ?? merged[0];
      if (fallback) merged = merged.map((account) => account.id === fallback.id ? { ...account, isDefault: true } : account);
    }
    try {
  ...
  const saveFloatEditor = async () => {
    ...
    for (const item of channelRows) {
      ...
      if (item.kind === "cliq" && (!item.detail.trim() || !item.provider.trim())) { Alert.alert(language === "ar" ? "بيانات CliQ مطلوبة" : "CliQ details required", language === "ar" ? "أدخل الاسم المستعار أو رقم الموبايل واسم البنك أو المحفظة." : "Enter the alias/mobile number and the bank or wallet name."); return; }
      if (item.kind === "bank" && (!item.provider.trim() || !item.iban.trim() || item.iban.trim().length < 15)) { Alert.alert(language === "ar" ? "بيانات الحساب البنكي غير مكتملة" : "Incomplete bank account details", language === "ar" ? "أدخل اسم البنك ورقم الآيبان الدولي (15 خانة على الأقل)." : "Enter the bank name and an international IBAN (15+ characters)."); return; }
    }
    const channels: StaffFloatChannel[] = channelRows.filter((item) => item.detail.trim() || item.provider.trim() || item.holderName.trim() || item.iban.trim()).map((item) => ({ kind: (item.kind === "cliq" || item.kind === "bank") ? item.kind : "cliq", alias: (item.kind === "cliq" ? item.detail.trim() : "").slice(0, 160) || undefined, provider: item.provider.trim().slice(0, 60) || undefined, holderName: item.holderName.trim().slice(0, 60) || undefined, iban: (item.kind === "bank" ? item.iban.trim() : "").slice(0, 200) || undefined, isDefault: item.isDefault || undefined, whatsApp: item.whatsApp || undefined, isActive: item.isActive === false ? false : undefined }));
  ...
  const isChannelRowComplete = (group: "cliq" | "bank" | "vault", row: OwnerRow): boolean => group === "vault" ? Boolean(row.label.trim()) : group === "cliq" ? isChannelDataComplete("cliq", { aliasOrPhone: row.detail, bankOrWallet: row.provider, bankName: "", iban: "" }) : isChannelDataComplete("bank", { aliasOrPhone: "", bankOrWallet: "", bankName: row.provider, iban: row.iban });
  const isOwnerTreasuryComplete = (account: OwnerTreasuryAccount): boolean => {
    if (account.kind === "cliq") return isChannelDataComplete("cliq", { aliasOrPhone: account.detail ?? "", bankOrWallet: account.provider ?? "", bankName: "", iban: "" });
    if (account.kind === "bank") return isChannelDataComplete("bank", { aliasOrPhone: "", bankOrWallet: "", bankName: account.provider ?? "", iban: (account.detail || account.iban) ?? "" });
    return account.isActive !== false;
  };
  const isChannelComplete = (channel: StaffFloatChannel): boolean => channel.isActive === false || (channel.kind === "cliq" ? isChannelDataComplete("cliq", { aliasOrPhone: channel.alias ?? "", bankOrWallet: channel.provider ?? "", bankName: "", iban: "" }) : isChannelDataComplete("bank", { aliasOrPhone: "", bankOrWallet: "", bankName: channel.provider ?? "", iban: channel.iban ?? "" }));
  const isFloatComplete = (account: StaffFloatAccount): boolean => {
    ...
    return channels.every(isChannelComplete);
  };
  ...
  type AccountChip = { key: string; title: string; subtitle: string; active: boolean; whatsapp: boolean; isDefault: boolean; incomplete: boolean };

  const ownerAccountChips = (account: OwnerTreasuryAccount): AccountChip[] => {
    const provider = (account.provider || "").trim();
    const holder = (account.holderName || "").trim();
    const detail = (account.kind === "bank" ? (account.detail || account.iban || "") : account.detail || "").trim();
    const incomplete = !isOwnerTreasuryComplete(account);
    let title: string;
    if (account.kind === "cliq") title = `${provider || account.label}${detail ? ` • ${detail}` : (language === "ar" ? " • ⚠️ بدون معرف" : " • ⚠️ No identifier")}${holder ? ` • ${holder}` : (language === "ar" ? " • بدون اسم" : " • No name")}`;
    else if (account.kind === "bank") title = `${provider || account.label}${detail ? ` • ${detail}` : (language === "ar" ? " • ⚠️ بدون معرف" : " • ⚠️ No identifier")}${holder ? ` • ${holder}` : ""}`;
    else title = account.label;
    return [{ key: account.id, title, subtitle: account.label, active: account.isActive !== false, whatsapp: account.whatsApp === true, isDefault: account.isDefault === true, incomplete }];
  };

  const floatAccountChips = (account: StaffFloatAccount): AccountChip[] => {
    const channels = Array.isArray(account.channels) && account.channels.length ? account.channels : [];
    const noName = language === "ar" ? "بدون اسم" : "No name";
    const missing = language === "ar" ? "⚠️ بدون معرف" : "⚠️ No identifier";
    if (!channels.length) return [{ key: account.id, title: `${account.label}${(account.cliqAlias || account.bankDetails || "") ? ` • ${(account.cliqAlias || account.bankDetails || "").split("\n")[0]}` : ""}`, subtitle: account.entity === "guard" ? (language === "ar" ? "نقطة حارس ميداني" : "Field guard point") : (language === "ar" ? "نقطة موظف" : "Staff collection point"), active: account.isActive !== false, whatsapp: account.whatsApp === true, isDefault: account.isDefault === true, incomplete: !isFloatComplete(account) }];
    return channels.map((channel) => {
      const provider = (channel.provider || "").trim();
      const holder = (channel.holderName || "").trim();
      const idfTrim = ((channel.kind === "cliq" ? channel.alias : channel.iban) || "").trim();
      const kindLabel = channel.kind === "cliq" ? "CliQ" : (language === "ar" ? "بنك" : "Bank");
      const title = `${provider || kindLabel}${idfTrim ? ` • ${idfTrim}` : ` • ${missing}`}${holder ? ` • ${holder}` : (channel.kind === "cliq" ? ` • ${noName}` : "")}`;
      return { key: `${account.id}-${channel.kind}-${channel.alias || channel.iban || "x"}`, title, subtitle: account.label, active: channel.isActive !== false, whatsapp: channel.whatsApp === true, isDefault: channel.isDefault === true, incomplete: !isChannelComplete(channel) };
    });
  };

  const renderAccountChip = (chip: AccountChip) => <View key={chip.key} style={[styles.chipRow, { backgroundColor: colors.background, borderColor: colors.border, flexDirection: "row" }]}><View style={[styles.dot, { backgroundColor: chip.active ? colors.success : colors.warning }]} /><View style={styles.flex}><Text numberOfLines={1} style={{ color: colors.foreground, fontSize: 10.5, fontWeight: "800", fontFamily: MONO_FONT, textAlign: align }}>{chip.title}</Text><Text numberOfLines={1} style={{ color: colors.muted, fontSize: 10, marginTop: 1, textAlign: align }}>{chip.subtitle}</Text></View><View style={[styles.channelTitleRow, { flexDirection: row }]}>{chip.incomplete ? renderStatusPill(language === "ar" ? "⚠️ بيانات غير مكتملة" : "⚠️ Incomplete data", colors.warning) : null}{renderStatusPill(chip.active ? (language === "ar" ? "مفعّل" : "Active") : (language === "ar" ? "معطّل" : "Inactive"), chip.active ? colors.success : colors.warning)}{chip.whatsapp ? renderStatusPill(language === "ar" ? "واتساب" : "WhatsApp", "#0EA5E9") : null}{chip.isDefault ? renderStatusPill(language === "ar" ? "★ افتراضي" : "★ Default", colors.primary, "star") : null}</View></View>;
  ...
  const ownerRowEditor = (group: "cliq" | "bank" | "vault", source: "owner" | "float") => {
    ...
    const setField = (key: string, patch: Partial<OwnerRow>) => { setRowErrors((prev) => { if (!prev[key]) return prev; const next = { ...prev }; delete next[key]; return next; }); setRows((current) => current.map((entry) => entry.key === key ? { ...entry, ...patch } : entry)); };
    return rows.map((item, index) => {
      ...
      return <View key={item.key} style={[styles.rowSubCard, { backgroundColor: colors.surfaceMuted + "66", borderColor: colors.border, marginTop: index === 0 ? 0 : 8 }]}>
        <View style={[styles.rowSubHead, { flexDirection: row }]}><Text style={{ color: colors.muted, fontSize: 11, fontWeight: "800" }}>{title}</Text><View style={styles.flex} /></View>
        ...
        <View style={[styles.subToolbar, { borderTopColor: colors.border }]}>{rows.length > 1 ? <Pressable accessibilityRole="button" accessibilityLabel={`${language === "ar" ? "حذف الحساب" : "Remove account"}`} onPress={() => setRows((current) => current.filter((entry) => entry.key !== item.key))} style={({ pressed }) => [styles.subToolbarTrash, { backgroundColor: colors.error + "10", borderColor: colors.error + "45", opacity: pressed ? 0.7 : 1 }]}><MaterialIcons name="delete-outline" size={16} color={colors.error} /></Pressable> : null}<View style={styles.flex} /><View style={styles.subToolbarPills}><Pressable accessibilityRole="checkbox" accessibilityState={{ checked: item.isActive }} onPress={() => { if (!item.isActive && !isChannelRowComplete(group, item)) { hintRow(item.key, group === "cliq" ? (language === "ar" ? "يرجى ملء الاسم المستعار واسم المحفظة لتفعيل هذا الحساب" : "Fill the alias and the wallet/bank name to activate this account") : (language === "ar" ? "يرجى ملء اسم البنك ورقم الآيبان (15 خانة على الأقل) لتفعيل هذا الحساب" : "Fill the bank name and IBAN (15+ characters) to activate this account")); return; } setField(item.key, item.isActive ? { isActive: false, whatsApp: false } : { isActive: true }); }} style={({ pressed }) => [styles.rowDefaultPill, { borderColor: item.isActive ? colors.success : colors.border, backgroundColor: item.isActive ? colors.success + "14" : "transparent", opacity: pressed ? 0.7 : 1, flexDirection: row, marginTop: 0 }]}><MaterialIcons name={item.isActive ? "check-box" : "check-box-outline-blank"} size={13} color={item.isActive ? colors.success : colors.muted} /><Text style={{ color: item.isActive ? colors.success : colors.muted, fontSize: 10.5, fontWeight: "800" }}>{language === "ar" ? "تفعيل هذا الحساب" : "Activate this account"}</Text></Pressable>{group !== "vault" ? <Pressable accessibilityRole="checkbox" accessibilityState={{ checked: item.whatsApp, disabled: !item.isActive }} onPress={() => { if (!item.isActive) { showToast(language === "ar" ? "يجب تفعيل الحساب أولاً لتضمينه في الرسائل" : "Activate the account first to include it in messages"); return; } setField(item.key, { whatsApp: !item.whatsApp }); }} style={({ pressed }) => [styles.rowDefaultPill, { borderColor: item.whatsApp ? "#25D366" : colors.border, backgroundColor: item.whatsApp ? "#25D366" + "14" : "transparent", opacity: item.isActive ? (pressed ? 0.7 : 1) : 0.5, flexDirection: row, marginTop: 0 }]}><MaterialIcons name={item.whatsApp ? "check-box" : "check-box-outline-blank"} size={13} color={item.isActive ? (item.whatsApp ? "#25D366" : colors.muted) : colors.muted} /><Text style={{ color: item.isActive ? (item.whatsApp ? "#25D366" : colors.muted) : colors.muted, fontSize: 10.5, fontWeight: "800" }}>{language === "ar" ? "الظهور في قوالب الرسائل (واتساب)" : "Show in WhatsApp templates"}</Text></Pressable> : null}<Pressable accessibilityRole="radio" accessibilityState={{ checked: item.isDefault }} onPress={() => { if (item.isDefault) { setRows((current) => current.map((entry) => entry.key === item.key ? { ...entry, isDefault: false } : entry)); showToast(ownerMode ? (language === "ar" ? "✓ تم إرجاع الحساب الافتراضي إلى حساب المالك الأساسي" : "✓ Default reverted to the owner's baseline account") : (language === "ar" ? "تم إلغاء تعيين الافتراضي" : "Default unset")); } else { pickRowDefault(item.key); } }} style={({ pressed }) => [styles.rowDefaultPill, { borderColor: item.isDefault ? colors.primary : colors.border, backgroundColor: item.isDefault ? colors.primary + "14" : "transparent", opacity: pressed ? 0.7 : 1, flexDirection: row, marginTop: 0 }]}><MaterialIcons name={item.isDefault ? "star" : "star-border"} size={13} color={item.isDefault ? colors.primary : colors.muted} /><Text style={{ color: item.isDefault ? colors.primary : colors.muted, fontSize: 10.5, fontWeight: "800" }}>{language === "ar" ? "تعيين كافتراضي ★" : "Set as default ★"}</Text></Pressable></View></View>
  ...
  const pickEntity = (entity: "owner" | "staff" | "guard") => {
    ...
    if (entity === "owner") {
      ...
      if (ownerEmpty) { setOwnerCliqRows([emptyOwnerRow("cliq")]); setOwnerVaultRows([]); setOwnerBankRows([]); setEditingAccountId(null); }
    } else {
      setOpenChannels({ cliq: false, bank: false, cash: true, other: false });
  ...
  return <ScreenContainer edges={["top", "bottom", "left", "right"]}><ScrollView style={{ flex: 1, backgroundColor: "transparent" }} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
    ...
    <View style={[styles.treasuryBody, { backgroundColor: colors.surface, borderColor: colors.border }]}><View style={[styles.badge, { backgroundColor: colors.primary + "0F", borderColor: colors.success + "3A", flexDirection: row }]}><MaterialIcons name="verified-user" size={17} color={colors.success} /><Text style={[styles.flex, { color: colors.muted, fontSize: 11.5, lineHeight: 18, fontWeight: "700", textAlign: align }]}>{language === "ar" ? "تستقر الدفعات مباشرة في إيرادات الخزينة العامة للمنشأة" : "Funds settle directly into the business general treasury"}</Text></View>
      {ownerAccounts.length ? <View style={styles.list}>{ownerAccounts.map((account) => { const soleActive = account.isActive !== false && activeOwnerCount <= 1; const solePrimaryVault = account.kind === "vault" && account.isDefault === true && ownerAccounts.length === 1; const kind = ownerKindMeta(account.kind); const accountChips = ownerAccountChips(account); return <View key={account.id} style={[styles.channelRow, { backgroundColor: colors.background, borderColor: colors.border, flexDirection: cardRow, direction: "ltr" }]}><View style={[styles.channelIcon, { backgroundColor: colors.primary + "14", borderColor: colors.primary + "35" }]}><MaterialIcons name={kind.icon} size={20} color={colors.primary} /></View><View style={styles.flex}><View style={[styles.channelTitleRow, { flexDirection: row }]}><Text numberOfLines={1} style={{ color: colors.foreground, fontSize: 14, fontWeight: "900", textAlign: align }}>{account.label}</Text>{accountChips.length ? renderStatusPill(language === "ar" ? `${accountChips.length} ${accountChips.length === 1 ? "حساب مسجل" : "حسابات مسجلة"}` : `${accountChips.length} registered account${accountChips.length === 1 ? "" : "s"}`, colors.muted) : null}{!isOwnerTreasuryComplete(account) ? renderStatusPill(language === "ar" ? "⚠️ بيانات غير مكتملة" : "⚠️ Incomplete data", colors.warning) : account.isActive !== false ? renderStatusPill(language === "ar" ? "✓ مفعّل وجاهز" : "✓ Active", colors.success, "check-circle") : null}{account.isDefault ? renderStatusPill(language === "ar" ? "افتراضي" : "Default", colors.success, "star") : null}{account.whatsApp ? renderStatusPill(language === "ar" ? "واتساب" : "WhatsApp", "#0EA5E9") : null}</View><Text numberOfLines={1} style={{ color: account.isActive !== false ? colors.muted : colors.warning, fontSize: 10.5, fontWeight: "700", marginTop: 3, textAlign: align, fontFamily: MONO_FONT }}>{ownerDetailLabel(account)}</Text></View><View style={[styles.channelControls, { flexDirection: row }]}><Pressable accessibilityRole="button" accessibilityLabel={`${language === "ar" ? "استعراض" : "Toggle"} ${account.label}`} onPress={() => toggleExpanded(account.id)} style={({ pressed }) => [styles.iconButton, { backgroundColor: colors.background, borderColor: colors.border, opacity: pressed ? 0.68 : 1 }]}><MaterialIcons name={expandedIds.has(account.id) ? "keyboard-arrow-up" : "keyboard-arrow-down"} size={17} color={colors.muted} /></Pressable><Pressable accessibilityRole="button" accessibilityLabel={`${language === "ar" ? "تعديل" : "Edit"} ${account.label}`} onPress={() => openOwnerEdit(account)} style={({ pressed }) => [styles.iconButton, { backgroundColor: colors.background, borderColor: colors.border, opacity: pressed ? 0.68 : 1 }]}><MaterialIcons name="edit" size={16} color={colors.primary} /></Pressable>{!solePrimaryVault ? <Pressable accessibilityRole="button" accessibilityLabel={`${language === "ar" ? "حذف" : "Remove"} ${account.label}`} onPress={() => removeOwner(account)} style={({ pressed }) => [styles.iconButton, { backgroundColor: colors.error + "10", borderColor: colors.error + "45", opacity: pressed ? 0.68 : 1 }]}><MaterialIcons name="delete-outline" size={16} color={colors.error} /></Pressable> : null}{canManage ? <AppToggle value={account.isActive !== false} onValueChange={(nextActive) => toggleOwnerActive(account, nextActive)} disabled={!isOwnerTreasuryComplete(account) || (account.isActive !== false && soleActive)} isRTL={isRTL} activeColor={colors.primary} inactiveColor={colors.border} accessibilityLabel={`${language === "ar" ? "تفعيل" : "Enable"} ${account.label}`} /> : null}</View>{expandedIds.has(account.id) ? <View style={[styles.chipTray, { backgroundColor: colors.surfaceMuted + "44", borderColor: colors.border, gap: 6 }]}>{accountChips.map(renderAccountChip)}</View> : null}</View>; })}</View> : null}
    </View>
...
const styles = StyleSheet.create({
  ...
  chipTray: { borderWidth: 1, borderRadius: 13, padding: 9, marginTop: 10 },
