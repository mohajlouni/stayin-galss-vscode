...
export default function PaymentMethodsScreen() {
  ...
  const [appMemberSearch, setAppMemberSearch] = useState("");
...
const missingIdLabel = () => language === "ar" ? "⚠️ بدون معرف" : "⚠️ No identifier";
  ...
  const onbookSuggestedUid = suggestOnbookUid(unifiedEntity === "guard" ? "guard" : "staff", [selectableMembers.map((member) => member.userCode), onbookStaff.map((entry) => entry.uid)].flat());
  const appMemberQuery = appMemberSearch.trim().toLocaleLowerCase();
  const filteredAppMembers = appMemberQuery ? selectableMembers.filter((member) => member.displayName.toLocaleLowerCase().includes(appMemberQuery) || (member.phone ?? "").toLocaleLowerCase().includes(appMemberQuery) || (member.userCode ?? "").toLocaleLowerCase().includes(appMemberQuery)) : [];
...
const styles = StyleSheet.create({
  ...
  uidBadge: { borderRadius: 12, borderWidth: 1, paddingHorizontal: 11, paddingVertical: 9, alignItems: "center", gap: 7 },
