...
type ConfirmationAction = "password" | "signout" | "reset-operations";
type SecurityIcon = "key" | "delete-forever" | "logout" | "cleaning-services";

export default function AccountSecurityScreen() {
  const colors = useColors();
  const { language, isRTL } = useI18n();
  const { logout, activeSession, biometricAvailable, setBiometricsEnabled, setRememberMe } = useAuthSession();
  ...
  const managePassword = async () => {
    ...
    try {
      ...
      setActionError(language === "ar" ? "تعذر فتح بوابة الهوية. تحقق من اتصال الإنترنت ثم أعد المحاولة." : "Could not open the identity portal. Check your connection and try again.");
    } finally {
      setActionBusy(false);
    }
  };
  ...
  const confirmationTitle = confirmation === "password"
    ? (language === "ar" ? "إدارة كلمة المرور" : "Manage password")
  ...
  const confirmationText = confirmation === "password"
    ? (language === "ar" ? "ستفتح بوابة الهوية الآمنة لتعيين كلمة المرور أو إعادة تعيينها. لا تُخزن كلمة المرور داخل التطبيق." : "The secure identity portal will open to set or reset your password. No password is stored in the app.")
    : confirmation === "reset-operations"
  ...
  const confirmLabel = confirmation === "password"
    ? (language === "ar" ? "فتح البوابة" : "Open portal")
    : confirmation === "reset-operations"
      ? (language === "ar" ? "نعم، تصفير السجلات" : "Yes, reset records")
      : (language === "ar" ? "تسجيل الخروج" : "Sign out");
  const confirmAction = async () => {
    if (confirmation === "password") { setConfirmation(null); await managePassword(); }
    else if (confirmation === "reset-operations") await resetOperations();
