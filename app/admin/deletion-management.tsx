...
export default function DeletionManagementScreen() {
  ...
  const previewProtected = previewData ? isSuperAdminUser({ email: previewData.user.email, phone: previewData.user.phone }) : false;
  const isProtectedAccount = (email: string | null | undefined, phone: string | null | undefined) => isSuperAdminUser({ email, phone });
  ...
  const isAuthDenied = Boolean(pending.error) && !pending.isLoading && pending.error?.data?.code === "FORBIDDEN";
  const loadError = pending.error && !pending.isLoading && !isAuthDenied && pending.error?.data?.code !== "UNAUTHORIZED";
  ...
  if (pending.error?.data?.code === "UNAUTHORIZED") return <AdminSessionExpired />;
  ...
  return <ScreenContainer edges={["top", "bottom", "left", "right"]}><ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
    <CompactScreenHeader title="إدارة الحذف النهائي والمهلة الزمنية" icon="delete-forever" accentColor={colors.error} backHref="/admin/master-control" showDateTime={false} />
    ...
    <View style={styles.metrics}><Metric label="محذوفة نهائياً" value={String(archived.data?.length ?? 0)} color={colors.primary} /><Metric label="قيد المهلة (14 يوماً)" value={String(pendingCount.data ?? pending.data?.length ?? 0)} color={colors.error} /></View>
...
const styles = StyleSheet.create({
  ...
  collapseToggle: { minHeight: 42, borderRadius: 11, borderWidth: 1, paddingHorizontal: 10, flexDirection: "row-reverse", alignItems: "center", justifyContent: "space-between", gap: 6 },
  ...
  miniLock: { minHeight: 34, borderRadius: 10, borderWidth: 1, paddingHorizontal: 10, alignItems: "center", justifyContent: "center", flexDirection: "row-reverse", gap: 4 },
  lockedRow: { minHeight: 52, borderRadius: 13, borderWidth: 1, paddingHorizontal: 11, flexDirection: "row-reverse", alignItems: "center", gap: 8 },
  lockedText: { flex: 1, fontSize: 11, lineHeight: 17, textAlign: "right", fontWeight: "800" },
