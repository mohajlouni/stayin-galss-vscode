...
function GlassTabIcon({ name, color, focused, size }: { name: TabIconName; color: string; focused: boolean; size: number }) {
  return <View style={[styles.tabIconWrap, focused && [styles.activeIconHalo, { backgroundColor: color + "1A" }]]}><IconSymbol name={name} size={size} color={color} />{focused ? <View style={[styles.activeDot, { backgroundColor: color }]} /> : null}</View>;
}
