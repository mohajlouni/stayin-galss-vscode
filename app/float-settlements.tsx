...
import { CommissionBreakdownModal } from "@/components/floats/CommissionBreakdownModal";
...
import { staffFloatCommissionAudit, staffFloatStatements, staffFloatStatementsForUser, type StaffFloatStatement } from "@/lib/reporting";
...
export default function FloatSettlementsScreen() {
  ...
  const [auditFor, setAuditFor] = useState<{ floatId: string; label: string; memberName?: string } | null>(null);
  ...
  const commissionAudits = useMemo(() => {
    const map = new Map<string, ReturnType<typeof staffFloatCommissionAudit>>();
    for (const statement of statements) map.set(statement.float.id, staffFloatCommissionAudit({ bookings, staffFloatSettlements, settings }, statement.float.id));
    return map;
  }, [bookings, staffFloatSettlements, settings, statements]);
  ...
  return <ScreenContainer edges={["top", "bottom", "left", "right"]}><ScrollView style={{ flex: 1, backgroundColor: "transparent" }} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
    ...
    {activeStatements.length ? activeStatements.map((statement) => {
      ...
      return <View key={account.id} style={[styles.floatCard, { backgroundColor: colors.surface, borderColor: colors.border }]}>
        ...
        {statement.commissionEarned > 0.005 ? <Pressable accessibilityRole="button" accessibilityLabel={language === "ar" ? "عرض كشف وتفصيل العمولات" : "View commissions breakdown"} onPress={() => setAuditFor({ floatId: account.id, label: account.label, memberName: account.memberName })} style={({ pressed }) => [styles.commissionRow, { backgroundColor: "#8B5CF6" + "12", borderColor: "#8B5CF6" + "4D", flexDirection: row, opacity: pressed ? 0.72 : 1 }]}><MaterialIcons name="payments" size={15} color="#8B5CF6" /><Text style={[styles.flex, { color: "#8B5CF6", fontSize: 10.5, fontWeight: "900", textAlign: align }]}>{language === "ar" ? `عمولات مستحقة للموظف (Commissions Earned): ${formatMoney(statement.commissionEarned, settings.currency)}` : `Commissions earned by staff: ${formatMoney(statement.commissionEarned, settings.currency)}`}</Text><MaterialIcons name={isRTL ? "chevron-left" : "chevron-right"} size={16} color="#8B5CF6" /></Pressable> : null}
        {canSettle ? <>
        ...
        <RipplePressable rippleColor={colors.background + "3D"} onPress={() => setAuditFor({ floatId: account.id, label: account.label, memberName: account.memberName })} style={({ pressed }) => [styles.ledgerButton, { backgroundColor: "transparent", borderColor: "#8B5CF6" + "66", borderWidth: 1, opacity: pressed ? 0.72 : 1 }]}><MaterialIcons name="fact-check" size={16} color="#8B5CF6" /><Text style={{ color: "#8B5CF6", fontWeight: "900", fontSize: 12 }}>{language === "ar" ? "كشف وتفصيل العمولات 📑" : "Commissions breakdown 📑"}</Text></RipplePressable>
    ...
    <CommissionBreakdownModal
      visible={Boolean(auditFor)}
      floatId={auditFor?.floatId ?? ""}
      floatLabel={auditFor?.label ?? ""}
      memberName={auditFor?.memberName}
      currency={settings.currency}
      audit={auditFor ? (commissionAudits.get(auditFor.floatId) ?? { rows: [], totalEarned: 0, settledEarned: 0, pendingEarned: 0 }) : { rows: [], totalEarned: 0, settledEarned: 0, pendingEarned: 0 }}
      onClose={() => setAuditFor(null)}
      onOpenBooking={(bookingId) => { setAuditFor(null); router.push({ pathname: "/booking-detail", params: { id: bookingId } } as never); }}
    />
