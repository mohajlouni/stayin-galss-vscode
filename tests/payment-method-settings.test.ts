...
describe("payment-method settings", () => {
  ...
  it("provides CRUD management and sends active customized methods into both booking collection selectors", () => {
    ...
    expect(management).toContain("تفعيل هذا الحساب");
    ...
    expect(management).toContain("يرجى ملء الاسم المستعار واسم المحفظة لتفعيل هذا الحساب");
    expect(management).toContain("بيانات الحساب البنكي غير مكتملة");
    expect(management).toContain("⚠️ بدون معرف");
    expect(management).toContain("isChannelDataComplete");
    expect(management).toContain("isChannelRowComplete");
    expect(management).toContain("iban.trim().length >= 15");
    expect(management).toContain("data.bankName.trim().length > 0 && data.iban.trim().length >= 15");
    ...
    expect(management).toContain("accessibilityState={{ checked: item.whatsApp, disabled: !item.isActive }}");
