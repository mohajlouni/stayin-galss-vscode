...
type OwnerRow = { key: string; id?: string; kind: OwnerTreasuryKind; label: string; detail: string; provider: string; holderName: string; iban: string; isDefault: boolean; whatsApp: boolean };
...
const emptyOwnerRow = (kind: OwnerTreasuryKind): OwnerRow => ({ key: createId(), kind, label: "", detail: "", provider: "", holderName: "", iban: "", isDefault: false, whatsApp: false });
...
export default function PaymentMethodsScreen() {
  ...
  const [ownerFlags, setOwnerFlags] = useState<{ isActive: boolean }>({ isActive: true });
  ...
  const openOwnerEdit = (account: OwnerTreasuryAccount) => {
    ...
    setFloatCliqRows([]);
    setFloatBankRows([]);
    const base: OwnerRow = { ...emptyOwnerRow(account.kind), id: account.id, label: account.label, detail: account.kind === "bank" ? (account.iban ?? account.detail) : account.detail, provider: account.provider ?? "", holderName: account.holderName ?? (account.kind !== "vault" ? account.label : ""), iban: account.iban ?? (account.kind === "bank" ? account.detail : ""), isDefault: account.isDefault === true, whatsApp: account.whatsApp === true };
    setOwnerVaultRows(account.kind === "vault" ? [base] : []);
    setOwnerCliqRows(account.kind === "cliq" ? [base] : []);
    setOwnerBankRows(account.kind === "bank" ? [base] : []);
    setOwnerFlags({ isActive: account.isActive !== false });
  ...
  const openFloatEdit = (account: StaffFloatAccount) => {
    ...
    const cliqRows = channels.filter((c) => c.kind === "cliq").map((c) => ({ key: createId(), kind: "cliq" as const, label: "", detail: c.alias ?? "", provider: c.provider ?? "", holderName: c.holderName ?? "", iban: "", isDefault: c.isDefault === true, whatsApp: c.whatsApp === true }));
    const bankRows = channels.filter((c) => c.kind === "bank").map((c) => ({ key: createId(), kind: "bank" as const, label: "", detail: "", provider: c.provider ?? "", holderName: c.holderName ?? "", iban: c.iban ?? "", isDefault: c.isDefault === true, whatsApp: c.whatsApp === true }));
    setFloatCliqRows(cliqRows.length ? cliqRows : account.cliqAlias ? [{ key: createId(), kind: "cliq" as const, label: "", detail: account.cliqAlias, provider: "", holderName: "", iban: "", isDefault: false, whatsApp: account.whatsApp === true }] : []);
    setFloatBankRows(bankRows.length ? bankRows : account.bankDetails ? [{ key: createId(), kind: "bank" as const, label: "", detail: "", provider: "", holderName: "", iban: account.bankDetails, isDefault: false, whatsApp: account.whatsApp === true }] : []);
  ...
  const ownerRowEditor = (group: "cliq" | "bank" | "vault", source: "owner" | "float") => {
    ...
    return rows.map((item, index) => {
      ...
      return <View key={item.key} style={[styles.rowSubCard, { backgroundColor: colors.surfaceMuted + "66", borderColor: colors.border, marginTop: index === 0 ? 0 : 8 }]}>
        ...
        {group !== "vault" ? <Pressable accessibilityRole="checkbox" accessibilityState={{ checked: item.whatsApp }} onPress={() => setField(item.key, { whatsApp: !item.whatsApp })} style={({ pressed }) => [styles.rowDefaultPill, { borderColor: item.whatsApp ? "#25D366" : colors.border, backgroundColor: item.whatsApp ? "#25D366" + "14" : "transparent", opacity: pressed ? 0.7 : 1, flexDirection: row }]}><MaterialIcons name={item.whatsApp ? "check-box" : "check-box-outline-blank"} size={13} color={item.whatsApp ? "#25D366" : colors.muted} /><Text style={{ color: item.whatsApp ? "#25D366" : colors.muted, fontSize: 10.5, fontWeight: "800", flexShrink: 1 }}>{language === "ar" ? "تضمين هذا الحساب في رسائل الواتساب وقوالب الحجز للعميل" : "Include this account in client WhatsApp messages & booking templates"}</Text></Pressable> : null}
  ...
  const pickEntity = (entity: "owner" | "staff" | "guard") => {
    ...
    if (entity === "owner") {
      ...
      if (ownerEmpty) { setOwnerCliqRows([emptyOwnerRow("cliq")]); setOwnerVaultRows([]); setOwnerBankRows([]); setOwnerFlags({ isActive: true }); setEditingAccountId(null); }
    } else {
      setOpenChannels({ cliq: false, bank: false, cash: true, other: false });
