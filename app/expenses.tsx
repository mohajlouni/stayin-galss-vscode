...
import { ActivityIndicator, Alert, FlatList, Image, Modal, Platform, Pressable, ScrollView, StyleSheet, Text, TextInput, ToastAndroid, View, type GestureResponderEvent, type LayoutChangeEvent } from "react-native";
...
export default function ExpensesScreen() {
  ...
  const expenseStripDates = useMemo(() => {
    const out: string[] = [];
    let cursor = addDays(stripCenter, -7);
    while (out.length < 14) {
      out.push(cursor);
  ...
  const periodLabel = period === "today" ? `${language === "ar" ? "اليوم" : "Today"} · ${formatDate(todayAnchor)}` : period === "7" ? `${language === "ar" ? "خلال 7 أيام" : "Last 7 days"} · ${formatDate(addDays(weekAnchor, -6))} — ${formatDate(weekAnchor)}` : period === "month" ? `${language === "ar" ? "هذا الشهر" : "This month"} · ${formatMonth(monthAnchorYear, monthAnchorNumber)}` : period === "all" ? `${language === "ar" ? "كل السجل" : "All records"}` : `${formatDate(customStart)} — ${formatDate(customEnd)}`;
  const backLabel = period === "today" ? (language === "ar" ? "اليوم السابق" : "Previous day") : period === "7" ? (language === "ar" ? "السبع أيام السابقة" : "Previous 7 days") : (language === "ar" ? "الشهر السابق" : "Previous month");
  const forwardLabel = period === "today" ? (language === "ar" ? "اليوم التالي" : "Next day") : period === "7" ? (language === "ar" ? "السبع أيام التالية" : "Next 7 days") : (language === "ar" ? "الشهر التالي" : "Next month");
  ...
  const handlePrev = (event: GestureResponderEvent) => {
    event.stopPropagation();
    shiftPeriod(isRTL ? 1 : -1);
  };
  ...
  return <ScreenContainer edges={["top", "bottom", "left", "right"]}><View style={styles.screen}>
    ...
    <View style={[styles.dayStrip, { flexDirection: row }]}><Pressable accessibilityRole="button" accessibilityLabel={language === "ar" ? "تمرير الأيام للخلف" : "Scroll days backward"} onPress={(event) => { event?.stopPropagation?.(); nudgeStrip(-1); }} {...mouseClick(() => nudgeStrip(-1))} pointerEvents="auto" style={({ pressed }) => [styles.stripArrow, { backgroundColor: colors.surfaceMuted, borderColor: colors.border, opacity: pressed ? 0.6 : 1, position: "relative", zIndex: 30, elevation: 30, cursor: "pointer", userSelect: "none" }]}><MaterialIcons name={isRTL ? "chevron-right" : "chevron-left"} size={18} color="#F59E0B" /></Pressable><ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.stripContent}>{expenseStripDates.map((date) => { const selected = period === "today" && todayAnchor === date; const isToday = date === todayISO(); const hasExpense = scopedDateSet.has(date); const weekday = STRIP_WEEKDAYS[language === "ar" ? "ar" : "en"][new Date(`${date}T12:00:00Z`).getUTCDay()]; return <Pressable key={date} accessibilityRole="button" accessibilityLabel={language === "ar" ? `تاريخ ${date}` : `Date ${date}`} onPress={() => selectStripDay(date)} style={[styles.stripChip, selected ? { backgroundColor: "#F59E0B", borderColor: "#F59E0B" } : isToday ? { backgroundColor: "rgba(245, 158, 11, 0.18)", borderColor: "#F59E0B", borderWidth: 2 } : { backgroundColor: colors.surfaceMuted, borderColor: colors.border }]}><Text numberOfLines={1} style={{ color: selected ? "#FFFFFF" : isToday ? "#FDE68A" : "#94A3B8", fontSize: 9, fontWeight: selected || isToday ? "900" : "600", textAlign: "center" }}>{isToday ? (language === "ar" ? "اليوم" : "Today") : weekday}</Text><Text style={{ color: selected ? "#FFFFFF" : isToday ? "#FCD34D" : colors.foreground, fontSize: 13, fontWeight: selected || isToday ? "900" : "700", textAlign: "center" }}>{date.slice(8, 10)}</Text><View style={[styles.stripDot, { backgroundColor: hasExpense ? "#F59E0B" : "transparent" }]} /></Pressable>; })}</ScrollView><Pressable accessibilityRole="button" accessibilityLabel={language === "ar" ? "تمرير الأيام للأمام" : "Scroll days forward"} onPress={(event) => { event?.stopPropagation?.(); nudgeStrip(1); }} {...mouseClick(() => nudgeStrip(1))} pointerEvents="auto" style={({ pressed }) => [styles.stripArrow, { backgroundColor: colors.surfaceMuted, borderColor: colors.border, opacity: pressed ? 0.6 : 1, position: "relative", zIndex: 30, elevation: 30, cursor: "pointer", userSelect: "none" }]}><MaterialIcons name={isRTL ? "chevron-left" : "chevron-right"} size={18} color="#F59E0B" /></Pressable></View>
...
const styles = StyleSheet.create({
  screen: { flex: 1 }, navBar: { marginHorizontal: 16, marginTop: 12, alignItems: "center", justifyContent: "center", gap: 8 }, navChips: { flexDirection: "row", gap: 6, flexShrink: 1, alignItems: "center" }, periodNav: { marginHorizontal: 16, marginTop: 10, alignItems: "center", gap: 8 }, periodArrow: { width: 38, height: 38, borderRadius: 12, borderWidth: 1, alignItems: "center", justifyContent: "center", flexShrink: 0 }, headerWrap: { paddingHorizontal: 16, paddingTop: 8 },
  headerActions: { marginTop: 10, gap: 8 },
  headerAction: { minHeight: 34, borderRadius: 11, borderWidth: 1, paddingHorizontal: 12, alignItems: "center", gap: 6 },
  searchWrap: { marginHorizontal: 16, marginTop: 10 },
