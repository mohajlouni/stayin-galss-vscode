...
export default function ExpensesScreen() {
  ...
  /** مسح الفترة المخصصة والعودة إلى كل السجل. */
  ...
  return <ScreenContainer edges={["top", "bottom", "left", "right"]}><View style={styles.screen}>
    ...
    <View style={styles.timelineWrap}>
      <View style={styles.timelineChips}><Pressable accessibilityRole="button" accessibilityLabel={language === "ar" ? "فترة مخصصة" : "Custom range"} onPress={() => setRangePickerOpen(true)} style={[styles.rangeChip, { backgroundColor: period === "custom" ? "#EA580C" : colors.surface, borderColor: period === "custom" ? "#EA580C" : colors.border, shadowColor: period === "custom" ? "#EA580C" : "transparent", shadowOpacity: period === "custom" ? 0.4 : 0, shadowRadius: 6, shadowOffset: { width: 0, height: 2 }, elevation: period === "custom" ? 4 : 0, flexDirection: row }]}><MaterialIcons name="date-range" size={14} color={period === "custom" ? "#FFFFFF" : colors.muted} /><Text numberOfLines={1} style={{ maxWidth: 96, color: period === "custom" ? "#FFFFFF" : colors.muted, fontSize: 12, fontWeight: period === "custom" ? "900" : "700" }}>{period === "custom" ? `${formatDate(customStart)} — ${formatDate(customEnd)}` : (language === "ar" ? "من - إلى" : "From - To")}</Text>{period === "custom" ? <Pressable accessibilityRole="button" accessibilityLabel={language === "ar" ? "مسح الفترة" : "Clear range"} onPress={(event) => { event?.stopPropagation?.(); clearRangeFilter(); }} {...mouseClick(clearRangeFilter)} style={({ pressed }) => ({ opacity: pressed ? 0.6 : 1 })}><MaterialIcons name="close" size={13} color="#FFFFFF" /></Pressable> : null}</Pressable><ChoiceChip active={period === "today"} label={language === "ar" ? "اليوم" : "Today"} onPress={() => { setPeriod("today"); setTodayAnchor(todayISO()); }} colors={colors} isRTL={isRTL} /><ChoiceChip active={period === "7"} label={language === "ar" ? "خلال 7 أيام" : "Last 7 days"} onPress={() => { setPeriod("7"); setWeekAnchor(todayISO()); }} colors={colors} isRTL={isRTL} /><ChoiceChip active={period === "month"} label={language === "ar" ? "هذا الشهر" : "This month"} onPress={() => { setPeriod("month"); setMonthAnchor(todayISO().slice(0, 7)); }} colors={colors} isRTL={isRTL} /><ChoiceChip active={period === "all"} label={language === "ar" ? "الكل" : "All"} onPress={() => setPeriod("all")} colors={colors} isRTL={isRTL} /></View>
      <Text numberOfLines={1} style={{ color: colors.muted, fontSize: 11, textAlign: align }}>{periodLabel}</Text>
    ...
    <View style={[styles.filterRow, { flexDirection: "row", alignItems: "center", gap: 8, zIndex: scopeMenuOpen ? 2 : 0 }]}>
    ...
    {scopeMenuOpen ? <Pressable accessibilityRole="button" accessibilityLabel={language === "ar" ? "إغلاق" : "Close"} onPress={() => setScopeMenuOpen(false)} style={[StyleSheet.absoluteFill, styles.clickAway]} /> : null}
...
const styles = StyleSheet.create({
  screen: { flex: 1 }, timelineWrap: { marginHorizontal: 16, gap: 8 }, timelineChips: { flexDirection: "row", gap: 6, flexShrink: 1, alignItems: "center", flexWrap: "wrap" }, filterRow: { marginHorizontal: 16, marginTop: 10, flexDirection: "row", alignItems: "center", gap: 8 }, rangeChip: { minHeight: 38, borderRadius: 12, borderWidth: 1, paddingHorizontal: 10, alignItems: "center", gap: 5 }, scopeMenuAnchor: { position: "relative", flexShrink: 1, maxWidth: 175, minWidth: 110 }, toolbarSelect: { minHeight: 40, borderRadius: 12, borderWidth: 1, paddingHorizontal: 10, alignItems: "center", gap: 5 }, toolbarMenu: { position: "absolute", top: "100%", marginTop: 8, minWidth: 200, maxWidth: 280, borderRadius: 12, borderWidth: 1, overflow: "hidden", zIndex: 50 }, toolbarRow: { minHeight: 40, paddingHorizontal: 12, alignItems: "center", gap: 8 }, clickAway: { zIndex: 1, backgroundColor: "rgba(0, 0, 0, 0.2)" }, headerWrap: { paddingHorizontal: 16, paddingTop: 8 },
