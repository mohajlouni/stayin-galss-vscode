...
const paymentSchema = z.object({
  ...
  recipientEntityId: z.string().max(64).optional(),
  recipientSubAccountId: z.string().max(128).optional(),
  isCustody: z.boolean().optional(),
  settlementId: z.string().regex(/^[a-zA-Z0-9-]{2,72}$/).optional(),
...
const bookingSchema = z.object({
  id: identifierSchema,
  bookingReference: z.string().regex(/^(?:#\d{2}-\d{4,}|#[A-Z0-9\u0621-\u064A]{2,3}\d{7}|#[A-Z0-9]+-[A-Z0-9]+-[A-Z0-9]+-\d{6}-[A-Z])$/i).optional(),
  ...
  earnedCommission: moneySchema.optional(),
...
const chaletSchema = z.object({
  ...
  referenceCode: z.string().regex(/^[A-Z0-9\u0621-\u064A]{2,3}$/).optional(),
...
const settingsSchema = z.object({
  ...
  paymentRouting: z.object({ ownerAccounts: z.array(z.object({ id: z.string().regex(/^[a-z0-9-]{2,72}$/), kind: z.enum(["cliq", "bank", "vault", "other"]), label: z.string().min(1).max(60), detail: z.string().max(200).optional(), provider: z.string().max(60).optional(), holderName: z.string().max(60).optional(), iban: z.string().max(200).optional(), isActive: z.boolean().optional(), isDefault: z.boolean().optional(), whatsApp: z.boolean().optional() })).optional(), masterAccounts: z.object({ cliqAlias: z.string().max(160).optional(), bankDetails: z.string().max(1000).optional(), cashHandlerLabel: z.string().max(120).optional(), directCliqEnabled: z.boolean().optional(), directBankEnabled: z.boolean().optional(), directCashEnabled: z.boolean().optional() }).optional(), staffFloats: z.array(z.object({ id: z.string().regex(/^[a-z0-9-]{2,72}$/), memberUserId: z.number().int().positive().optional(), memberName: z.string().max(120).optional(), contactPhone: z.string().max(30).optional(), label: z.string().min(1).max(120), cliqAlias: z.string().max(160).optional(), bankDetails: z.string().max(1000).optional(), channels: z.array(z.object({ kind: z.enum(["cliq", "bank"]), alias: z.string().max(160).optional(), provider: z.string().max(60).optional(), holderName: z.string().max(60).optional(), iban: z.string().max(200).optional(), isDefault: z.boolean().optional(), whatsApp: z.boolean().optional(), isActive: z.boolean().optional() })).optional(), maxFloatLimit: moneySchema.optional(), isActive: z.boolean().optional(), isDefault: z.boolean().optional(), whatsApp: z.boolean().optional(), entity: z.enum(["staff", "guard"]).optional(), isCommissionEnabled: z.boolean().optional(), commissionType: z.enum(["FIXED_PER_BOOKING", "PERCENTAGE_OF_TOTAL"]).optional(), commissionValue: moneySchema.optional() })).optional() }).optional(),
...
const maintenanceTaskSchema = z.object({
  ...
  expenseFundingEntity: z.enum(["owner", "staff"]).optional(),
  expenseFundingChannel: z.enum(["vault-cash", "cliq", "iban"]).optional(),
  expenseFundingSourceLabel: z.string().max(120).optional(),
  expenseFundingMode: z.enum(["float", "reimbursement"]).optional(),
...
const expenseSchema = z.object({
  ...
  amount: moneySchema,
  date: dateSchema,
  category: z.enum(["guards-salaries", "maintenance", "cleaning-supplies", "utilities", "other", "fuel-gas", "hospitality", "commissions-bonuses"]),
  note: optionalText,
  paymentMethod: z.enum(["cash", "click", "iban"]).optional(),
  receiptUri: optionalText,
  ...
  maintenanceTaskId: z.string().regex(/^[a-zA-Z0-9-]{2,72}$/).optional(),
  fundingEntity: z.enum(["owner", "staff"]).optional(),
  fundingChannel: z.enum(["vault-cash", "cliq", "iban"]).optional(),
  fundingSourceId: identifierSchema.optional(),
  fundingSourceLabel: optionalText,
  isFloatExpense: z.boolean().optional(),
  isStaffReimbursement: z.boolean().optional(),
  isSettled: z.boolean().optional(),
  settlementId: z.string().regex(/^[a-zA-Z0-9-]{2,72}$/).optional(),
  isStaffReimbursementSettled: z.boolean().optional(),
  clearsReimbursementForFloatId: identifierSchema.optional(),
  reimbursementChannel: z.enum(["vault", "cliq", "bank"]).optional(),
  reimbursementSettled: z.boolean().optional(),
  reimbursementSettledById: z.string().regex(/^[a-zA-Z0-9-]{2,72}$/).optional(),
  createdAt: z.string(),
  createdByName: z.string().optional(),
...
const backupSchema = z.object({
  ...
  staffFloatSettlements: z.array(z.object({ id: z.string().regex(/^[a-zA-Z0-9-]{2,72}$/), floatId: z.string().regex(/^[a-zA-Z0-9-]{2,72}$/), amount: moneySchema, settledAt: z.string().datetime(), createdAt: z.string().datetime().optional(), settlementDate: dateSchema.optional(), recipientAccountId: z.string().regex(/^[a-zA-Z0-9-]{2,72}$/).optional(), recipientAccountLabel: z.string().max(120).optional(), channel: z.enum(["vault", "cliq", "bank"]).optional(), coveredPaymentIds: z.array(z.string()).optional(), coveredExpenseIds: z.array(z.string()).optional(), note: optionalText, settledByUserId: z.number().int().positive().optional(), settledByName: z.string().max(120).optional(), status: z.enum(["PENDING_APPROVAL", "CONFIRMED", "REJECTED"]).optional(), requestedByUserId: z.number().int().positive().optional(), requestedByName: z.string().max(120).optional(), receiptUri: z.string().max(600).optional(), rejectReason: z.string().max(400).optional(), approvedAt: z.string().datetime().optional(), approvedByUserId: z.number().int().positive().optional(), approvedByName: z.string().max(120).optional(), commissionOffset: moneySchema.optional(), staffBonus: moneySchema.optional() })).optional(),
  ...
  auditLog: z.array(z.object({ id: z.string(), action: z.enum(["waitlist-promoted", "waitlist-deleted", "waitlist-cancelled", "booking-deleted", "booking-cancelled", "booking-checked-in", "booking-checked-out", "booking-status-corrected", "booking-waitlist-priority-confirmed", "chalet-deleted", "payment-updated", "payment-voided", "customer-created", "customer-updated", "customer-blacklisted", "customer-unblacklisted", "contract-signed", "asset-added", "asset-updated", "asset-deleted", "maintenance-task-updated", "maintenance-task-completed", "weather-log-updated", "utility-reading-recorded", "loyalty-points-awarded", "loyalty-points-redeemed", "float-settled", "deposit-compensation-recorded", "staff-float-account-saved", "staff-reimbursement-paid", "float-settlement-requested", "float-settlement-approved", "float-settlement-rejected"]), subjectName: z.string(), details: z.string(), createdAt: z.string(), actorName: z.string().optional(), bookingId: z.string().optional() })).optional(),
