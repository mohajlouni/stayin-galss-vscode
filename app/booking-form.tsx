...
export default function BookingForm() {
  ...
  const { bookings, waitlist, chalets, settings, specialPriceRules, hydrated, addBooking, updateBooking, addWaitlist, promoteWaitlist, replaceConflictsAndSave } = useBookings();
  ...
  const draft = useMemo(() => {
    ...
    const depositCollection = !existing && depositValue > 0 && resolvedDepositMethod ? buildCollectionPayment("pending-security-deposit", depositValue, resolvedDepositMethod, depositRecipientId, language === "ar" ? "تأمين مسترد" : "Refundable security deposit") : existing?.depositCollection;
    return { id: existing?.id ?? "", customerName: name, phone: normalizedPhone.value ?? phone, chaletId: chaletId || undefined, chaletName: selectedChalet?.name ?? existing?.chaletName, startDate, endDate, bookingType: multiDayRange ? "multi-day" : bookingTypeForShift(shiftId), shiftId: selectedShift?.id, shiftName: selectedShift?.name, shiftColor: selectedShift?.color, startTime, endTime, price: rentalTotal, discountAmount: Number(discountAmount || 0), depositAmount: depositValue, depositPaymentMethod: resolvedDepositMethod, depositPaymentRecordedAt: resolvedDepositMethod ? existing?.depositPaymentRecordedAt ?? cloneSource?.depositPaymentRecordedAt ?? new Date().toISOString() : undefined, depositCollection, payments: existing?.payments ?? pendingInitialPayment, notes, status: existing?.status ?? "confirmed", createdAt: existing?.createdAt ?? new Date().toISOString() } as Booking;
  }, [automaticPrice, chaletId, cloneSource?.depositPaymentRecordedAt, depositAmount, depositPaymentMethod, depositRecipientId, discountAmount, endDate, existing, initialPayment, initialPaymentMethod, initialRecipientId, language, multiDayRange, name, normalizedPhone.value, notes, phone, price, priceIsManual, recipientChoices, selectedChalet?.name, selectedShift?.color, selectedShift?.id, selectedShift?.name, settings.paymentRouting?.masterAccounts?.bankDetails, settings.paymentRouting?.masterAccounts?.cashHandlerLabel, settings.paymentRouting?.masterAccounts?.cliqAlias, shiftId, startDate, startTime, endTime]);
  ...
  const inputStyle = [styles.input, { backgroundColor: colors.surfaceMuted, color: colors.foreground, textAlign: align as "left" | "right" }];
  ...
  return <ScreenContainer edges={["top", "bottom", "left", "right"]}><View style={styles.screen}><ScrollView style={styles.flex} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled"><View style={[styles.header, { flexDirection: row }]}><ScreenBackButton fallbackHref="/(tabs)/bookings" /><View style={styles.flex}><Text style={{ color: colors.foreground, fontSize: 25, fontWeight: "800", textAlign: align }}>{existing ? (language === "ar" ? "تعديل الحجز" : "Edit booking") : isWaitlistMode ? (language === "ar" ? "طلب انتظار جديد" : "New waitlist request") : t("addBooking")}</Text><Text style={{ color: colors.muted, fontSize: 12, marginTop: 3, textAlign: align }}>{isWaitlistMode ? (language === "ar" ? "أدخل تفاصيل الطلب كاملة؛ لن يُحسب حجزًا مؤكدًا حتى تحويله لاحقًا." : "Add the complete request details; it will not count as confirmed until promoted.") : (language === "ar" ? "أكمل الحقول المطلوبة ثم راجع الملخص." : "Complete the required fields, then review the summary.")}</Text></View></View>
    ...
    <FormSection title={t("customerData")} icon="person" colors={colors} row={row}>
      <View style={styles.field}>{label(t("customerName"), true)}<TextInput value={name} onChangeText={setName} placeholder={t("customerName")} placeholderTextColor={colors.muted} style={inputStyle} /></View>
      <View style={styles.field}>{label(t("phone"))}<View style={[styles.phoneRow, { flexDirection: row }]}><Pressable accessibilityRole="button" accessibilityLabel={language === "ar" ? "اختيار رمز الدولة" : "Choose country code"} onPress={() => setCountryPickerOpen(true)} style={({ pressed }) => [styles.countryCodeTrigger, { backgroundColor: colors.surfaceMuted, borderColor: colors.border, opacity: pressed ? 0.72 : 1 }]}><Text style={styles.countryFlag}>{COUNTRY_FLAGS[phoneCountry.iso]}</Text><Text style={{ color: colors.primary, fontSize: 12, fontWeight: "900", writingDirection: "ltr" }}>{phoneCountry.code}</Text><MaterialIcons name="keyboard-arrow-down" size={17} color={colors.muted} /></Pressable><TextInput value={phone} onChangeText={setPhone} placeholder={language === "ar" ? "79 000 0000" : "79 000 0000"} placeholderTextColor={colors.muted} keyboardType="phone-pad" returnKeyType="done" blurOnSubmit onSubmitEditing={() => Keyboard.dismiss()} textAlign="left" style={[inputStyle, styles.phoneInput]} /></View>{invalidPhone ? <Text style={{ color: colors.error, fontSize: 11, marginTop: 5, textAlign: align }}>{language === "ar" ? "أدخل رقمًا محليًا صحيحًا." : "Enter a valid local number."}</Text> : phone.trim() ? <Text style={{ color: colors.muted, fontSize: 10, marginTop: 5, textAlign: align, writingDirection: "ltr" }}>{normalizedPhone.value}</Text> : null}</View>
    ...
    <FormSection title={language === "ar" ? "الإيجار والتأمين" : "Rental and security deposit"} icon="payments" colors={colors} row={row}>
      <View style={[styles.rentSummaryRow, { flexDirection: rentRow }]}>
        ...
        <View style={styles.rentColumn}>{label(t("discount"))}<TextInput value={discountAmount} onChangeText={setDiscountAmount} placeholder={language === "ar" ? "اختياري" : "Optional"} placeholderTextColor={colors.muted} keyboardType="decimal-pad" style={inputStyle} /></View>
        <View style={styles.rentColumn}>{label(language === "ar" ? "سعر الإيجار" : "Base rent", true)}<TextInput value={price} onChangeText={(value) => { setPriceIsManual(true); setPrice(value); }} placeholder="0.00 د.أ" placeholderTextColor={colors.muted} keyboardType="decimal-pad" style={inputStyle} /><Text style={{ color: colors.muted, fontSize: 10, marginTop: 4, textAlign: align }}>{priceIsManual ? (language === "ar" ? "معدل يدويًا" : "Manual") : (language === "ar" ? "حسب الفترة والتاريخ" : "By period and date")}</Text>{!existing && priceIsManual ? <Pressable onPress={() => { setPriceIsManual(false); setPrice(automaticPrice > 0 ? String(automaticPrice) : ""); }}><Text style={{ color: colors.primary, fontWeight: "800", fontSize: 11, marginTop: 4, textAlign: align }}>{language === "ar" ? "استخدام السعر التلقائي" : "Use automatic price"}</Text></Pressable> : null}</View>
      </View>
      {!existing ? <View style={styles.field}>{label(language === "ar" ? "دفعة أولى من الإيجار (عربون)" : "Initial rental payment (deposit)")}<TextInput value={initialPayment} onChangeText={(value) => { setInitialPayment(value); if (Number(value || 0) <= 0) { setInitialPaymentMethod(null); setInitialPaymentMethodError(false); } }} placeholder={language === "ar" ? "اختيارية ولا تشمل التأمين" : "Optional; does not include security deposit"} placeholderTextColor={colors.muted} keyboardType="decimal-pad" style={inputStyle} /></View> : <Text style={{ color: colors.muted, fontSize: 12, lineHeight: 19, marginTop: 11, textAlign: align }}>{language === "ar" ? "لتسجيل دفعة جديدة من الإيجار، احفظ التعديل ثم استخدم «دفعة جديدة» من تفاصيل الحجز." : "To record a new rental payment, save these changes then use New payment in booking details."}</Text>}
      ...
      <View style={styles.field}>{label(language === "ar" ? "تأمين مسترد" : "Refundable security deposit")}<TextInput value={depositAmount} onChangeText={(value) => { setDepositAmount(value); if (Number(value || 0) <= 0) { setDepositPaymentMethod(null); setDepositPaymentMethodError(false); } }} placeholder={language === "ar" ? "اختياري" : "Optional"} placeholderTextColor={colors.muted} keyboardType="decimal-pad" style={inputStyle} /></View>
      ...
      <View style={styles.compactNotes}>{label(t("notes"))}<TextInput value={notes} onChangeText={setNotes} placeholder={t("notes")} placeholderTextColor={colors.muted} multiline textAlignVertical="top" style={[inputStyle, styles.notes]} /></View>
    </FormSection>
