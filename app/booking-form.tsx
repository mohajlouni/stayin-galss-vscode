...
export default function BookingForm() {
  ...
  const recipientChoices = useMemo<RecipientChoice[]>(() => {
    ...
    const ownerOptions: RecipientChoice[] = activeOwnerTreasuryAccounts(settings).map((account) => ({ id: `owner:${account.id}`, type: "owner", userId: owner?.userId ?? undefined, name: language === "ar" ? "حساب المالك الرئيسي" : "Master account", entityId: "owner", subAccountId: account.id, kind: account.kind, detail: account.detail, isDefault: account.isDefault === true, cashAvailable: account.kind === "vault", cliqAvailable: account.kind === "cliq", bankAvailable: account.kind === "bank", otherAvailable: account.kind === "other" }));
    const managerPaidReceivers: RecipientChoice[] = (collectionRecipients.data ?? []).filter((member) => member.role !== "owner").map((member) => ({ id: `member-${member.userId ?? ""}`, type: member.role === "guest" ? "guard" as const : "staff" as const, userId: member.userId ?? undefined, name: member.displayName, cliqAlias: member.cliqAlias, bankDetails: member.bankDetails, commissionRate: member.commissionRate, commissionType: member.commissionType, entityId: `member-${member.userId ?? ""}`, detail: member.cliqAlias ?? member.bankDetails ?? undefined, cashAvailable: true, cliqAvailable: Boolean(member.cliqAlias), bankAvailable: Boolean(member.bankDetails), otherAvailable: false }));
