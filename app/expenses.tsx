...
import { EXPENSE_CATEGORIES, EXPENSE_FUNDING_CHANNELS, EXPENSE_FUNDING_ENTITIES, activeOwnerTreasuryAccounts, activeStaffFloatAccounts, expenseFundingChannelLabel, expenseFundingEntityLabel, splitExpenseAcrossChalets, type Expense, type ExpenseCategory, type ExpenseFundingChannel, type ExpenseFundingEntity, type ExpensePaymentMethod, formatMoney, todayISO, weekdayLabel } from "@/lib/booking-model";
...
export default function ExpensesScreen() {
  const { expenses = [], chalets, settings, addExpense, updateExpense, deleteExpense } = useBookings();
  ...
  const scopeLabel = scope === "general" ? (language === "ar" ? "جميع الشاليهات / مصروف عام" : "All chalets / General expense") : scope ? (chalets.find((item) => item.id === scope)?.name ?? scope) : null;
  ...
  const visibleExpenses = useMemo<DisplayExpense[]>(() => {
    ...
    const periodNow = todayISO();
    const currentMonth = periodNow.slice(0, 7);
    return scoped.filter((expense) => {
      const date = expense.date || expense.createdAt.slice(0, 10);
      if (period === "today") return date === periodNow;
  ...
  }, [expenses, language, selectedChaletId, settings.currency, period, customStart, customEnd]);
  ...
  const periodLabel = period === "today" ? `${language === "ar" ? "اليوم" : "Today"} · ${formatDate(todayISO())}` : period === "month" ? (language === "ar" ? "هذا الشهر" : "This month") : `${formatDate(customStart)} — ${formatDate(customEnd)}`;
  ...
  const openEdit = (expense: Expense) => {
    ...
    setScope(expense.chaletId ?? "general");
  ...
  const remove = (expense: Expense) => Alert.alert(language === "ar" ? "حذف المصروف" : "Delete expense", language === "ar" ? (expense.maintenanceTaskId ? "هل أنت متأكد من حذف هذا المصروف نهائيًا؟ سيُفصل المصروف عن مهمة الصيانة وتُصفَّر تكلفتها إلى 0.00 JOD." : "هل أنت متأكد من حذف هذا المصروف نهائيًا؟") : (expense.maintenanceTaskId ? "Delete this expense permanently? It will be unlinked from the maintenance task and its cost reset to 0.00 JOD." : "Delete this expense permanently?"), [{ text: language === "ar" ? "إلغاء" : "Cancel", style: "cancel" }, { text: language === "ar" ? "حذف" : "Delete", style: "destructive", onPress: () => void deleteExpense(expense.id).catch(() => Alert.alert(language === "ar" ? "تعذر الحذف" : "Could not delete", language === "ar" ? "لا تملك صلاحية حذف المصروف." : "You do not have permission to delete this expense.")) }]);
