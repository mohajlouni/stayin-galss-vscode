...
import { findOnbookByPhone, findOnbookByUid, phoneKey, suggestOnbookUid, validateOnbookEntry, type OnbookStaff, type OnbookStaffRole } from "@/lib/staff-directory";
import { useOnbookStaff } from "@/lib/staff-directory-store";
import { useWorkspaceAccess } from "@/lib/workspace-access";
import { trpc } from "@/lib/trpc";
import type { WorkspaceAccessRole, WorkspacePermissions } from "@/shared/workspace-permissions";

const FIELD_BORDER = "#334155";
const FIELD_BG = "rgba(15, 23, 42, 0.8)";
const FIELD_FOCUS = "#F97316";
...
type FloatDraft = { id?: string; entity: "staff" | "guard"; onbookUid: string; label: string; uid: string; phone: string; hasOther: boolean; otherNote: string; ceiling: string; isActive: boolean; isDefault: boolean };
type TeamEntry =
  | { key: "owner"; kind: "owner"; icon: "account-balance"; name: string }
  | { key: string; kind: "member"; userId: number; icon: "person"; name: string; phone: string; userCode: string | null; roleLabel: string }
  | { key: string; kind: "onbook"; uid: string; icon: "badge"; name: string; phone: string; role: OnbookStaffRole; roleLabel: string };
...
const emptyFloatDraft = (entity: "staff" | "guard"): FloatDraft => ({ entity, onbookUid: "", label: "", uid: "", phone: "", hasOther: false, otherNote: "", ceiling: "", isActive: false, isDefault: false });
...
const memberRoleLabel = (role: WorkspaceAccessRole, language: "ar" | "en") => role === "admin" ? (language === "ar" ? "مدير تشغيلي" : "Operational manager") : role === "staff" ? (language === "ar" ? "موظف حجوزات" : "Booking staff") : role === "caretaker" ? (language === "ar" ? "حارس / مشرف ميداني" : "Caretaker / field supervisor") : role === "guest" ? (language === "ar" ? "ضيف / وصول محدود" : "Guest / limited access") : (language === "ar" ? "المالك" : "Owner");
...
export default function PaymentMethodsScreen() {
  ...
  const inviteEmployee = trpc.workspace.inviteEmployee.useMutation();
  ...
  const [teamChoice, setTeamChoice] = useState<TeamEntry | null>(null);
  const [teamPickerOpen, setTeamPickerOpen] = useState(false);
  const [addUserOpen, setAddUserOpen] = useState(false);
  const [focusedField, setFocusedField] = useState<"label" | "ceiling" | null>(null);
  ...
  const openOwnerEdit = (account: OwnerTreasuryAccount) => {
    ...
    setTeamChoice({ key: "owner", kind: "owner", icon: "account-balance", name: language === "ar" ? "الخزينة المركزية / المالك" : "Central treasury / Owner" });
  ...
  const openFloatEdit = (account: StaffFloatAccount) => {
    ...
    setUnifiedEntity(entity);
    setEditingAccountId(account.id);
    setTeamChoice(linkedOnbook ? { key: "onbook:" + linkedOnbook.uid, kind: "onbook", uid: linkedOnbook.uid, icon: "badge", name: linkedOnbook.name, phone: linkedOnbook.phone, role: linkedOnbook.role, roleLabel: onbookRoleLabel(linkedOnbook.role, language) } : memberMatch ? { key: "member:" + memberMatch.userId, kind: "member", userId: memberMatch.userId, icon: "person", name: memberMatch.displayName, phone: memberMatch.phone ?? "", userCode: memberMatch.userCode, roleLabel: language === "ar" ? "عضو الفريق" : "Team member" } : null);
    setOwnerVaultRows([]);
    setOwnerCliqRows([]);
    setOwnerBankRows([]);
    setOwnerOtherRows([]);
    ...
    setFloatDraft({ id: account.id, entity, onbookUid: linkedOnbook ? linkedOnbook.uid : "", label: account.label, uid: memberMatch?.userCode ?? account.memberUid ?? "", phone: account.contactPhone ?? "", hasOther: account.hasOther === true, otherNote: ((account.bankDetails ?? "").split("\n").find((line) => line.startsWith("أخرى: ")) ?? "").replace(/^أخرى: /, "").trim(), ceiling: account.maxFloatLimit !== undefined ? String(account.maxFloatLimit) : "", isActive: account.cashActive === true, isDefault: account.isDefault === true });
  ...
  const saveFloatEditor = async () => {
    ...
    if (!floatDraft.id && !floatDraft.phone.trim() && !floatDraft.onbookUid && !floatDraft.uid.trim()) { Alert.alert(language === "ar" ? "لا يمكن الحفظ بدون مسؤول" : "Responsible person required", language === "ar" ? "اختر المسؤول عن نقطة التحصيل أولًا من القائمة، أو أضِفه من إدارة المستخدمين." : "Pick the responsible person first, or add them in User Management."); return; }
    ...
    const onbookEntry = floatDraft.onbookUid ? findOnbookByUid(onbookStaff, floatDraft.onbookUid) : undefined;
...
const byPhoneOnbook = findOnbookByPhone(onbookStaff, floatDraft.phone);
if (byPhoneOnbook) { memberName = byPhoneOnbook.name; memberUid = byPhoneOnbook.uid; }
...
const byPhoneMember = selectableMembers.find((item) => item.phone && phoneKey(item.phone) === phoneKey(floatDraft.phone));
if (byPhoneMember) { memberUserId = byPhoneMember.userId; memberName = byPhoneMember.displayName; memberUid = byPhoneMember.userCode ?? undefined; }
...
const ownerSetDefault = (account: OwnerTreasuryAccount) => {
  ...
  const collectMissingTokens = (): string[] => {
    ...
    if (unifiedEntity !== "owner") {
      ...
      if (!floatDraft.phone.trim() && !floatDraft.onbookUid && !floatDraft.uid.trim()) tokens.push("ident:phone");
...
const missingIdLabel = () => language === "ar" ? "⚠️ بدون معرف" : "⚠️ No identifier";
  ...
  const ownerRowEditor = (group: "cliq" | "bank" | "vault" | "other", source: "owner" | "float") => {
    ...
    return rows.map((item, index) => { const active = item.isActive !== false; const ready = active && isChannelRowComplete(group, item);
      ...
      return group === "cliq" || group === "bank" ? <View key={item.key} onLayout={(event) => { layoutY.current["row-" + item.key] = event.nativeEvent.layout.y; }} style={[styles.compactChannelCard, { backgroundColor: FIELD_BG, borderColor: FIELD_BORDER, flexDirection: row, marginTop: index === 0 ? 0 : 8 }]}><View style={[styles.compactChannelActions, { flexDirection: "column" }]}>{renderDefaultStar(() => { if (item.isDefault) { setRows((current) => current.map((entry) => entry.key === item.key ? { ...entry, isDefault: false } : entry)); pushAudit("default", rowTargetLabel(item), language === "ar" ? "إلغاء التعيين الافتراضي" : "Unset as default"); showToast(ownerMode ? (language === "ar" ? "✓ تم إرجاع الحساب الافتراضي إلى حساب المالك الأساسي" : "✓ Default reverted to the owner's baseline account") : (language === "ar" ? "تم إلغاء تعيين الافتراضي" : "Default unset")); } else { if (!active) { hintRow(item.key, language === "ar" ? "لا يمكن تعيينه افتراضيًا — يجب تفعيل الحساب وإكمال بياناته أولًا" : "Can't set as default — activate the account and complete its data first"); return; } pickRowDefault(item.key); pushAudit("default", rowTargetLabel(item), language === "ar" ? `تعيين الحساب كافتراضي لطريقة ${groupLabel(group)}` : `Set account as default for ${groupLabel(group)}`); } }, !active, item.isDefault === true, colors.primary)}{rows.length > 1 ? <Pressable accessibilityRole="button" accessibilityLabel={language === "ar" ? "حذف الحساب" : "Remove account"} onPress={() => setRows((current) => current.filter((entry) => entry.key !== item.key))} style={({ pressed }) => [styles.compactChannelAction, { backgroundColor: colors.error + "10", borderColor: colors.error + "45", opacity: pressed ? 0.7 : 1 }]}><MaterialIcons name="delete-outline" size={16} color={colors.error} /></Pressable> : null}<Pressable accessibilityRole="checkbox" accessibilityState={{ checked: item.whatsApp, disabled: !active }} onPress={() => { if (!active) { showToast(language === "ar" ? "يجب تفعيل الحساب أولاً لتضمينه في الرسائل" : "Activate the account first to include it in messages"); return; } setField(item.key, { whatsApp: !item.whatsApp }); pushAudit("whatsapp", rowTargetLabel(item), language === "ar" ? `تعديل التضمين في قوالب الواتساب ${item.whatsApp ? "— إيقاف التفعيل في قوالب الواتساب" : "— تفعيل الظهور في قوالب الواتساب"}` : `WhatsApp template membership ${item.whatsApp ? "— disabled in WhatsApp templates" : "— enabled in WhatsApp templates"}`); }} style={({ pressed }) => [styles.compactChannelAction, { borderColor: item.whatsApp ? "#25D366" : colors.border, backgroundColor: item.whatsApp ? "#25D366" + "14" : "transparent", opacity: active ? (pressed ? 0.7 : 1) : 0.5 }]}><MaterialIcons name={item.whatsApp ? "check-box" : "check-box-outline-blank"} size={13} color={active ? (item.whatsApp ? "#25D366" : colors.muted) : colors.muted} /></Pressable></View><View style={styles.flex}><View style={[styles.channelTitleRow, { flexDirection: row }]}><Text numberOfLines={1} style={{ color: colors.foreground, fontSize: 12, fontWeight: "900", textAlign: align, fontFamily: MONO_FONT }}>{title}</Text><View style={styles.flex} />{ready ? renderStatusPill(language === "ar" ? "✓ مفعّل وجاهز" : "✓ Active", colors.success, "check-circle") : !active ? renderStatusPill(language === "ar" ? "معطّل" : "Inactive", colors.warning) : renderStatusPill(language === "ar" ? "⚠️ بانتظار استكمال البيانات" : "⚠️ Awaiting data completion", "#F59E0B")}</View>{fields.map((field) => <TextInput key={field.key} value={item[field.key]} onChangeText={(text) => setField(item.key, { [field.key]: text } as Partial<OwnerRow>)} maxLength={field.key === "iban" ? 40 : field.key === "detail" ? 200 : 60} placeholder={field.placeholder} placeholderTextColor={colors.muted} style={[styles.compactInput, field.mono ? styles.inputMono : null, { backgroundColor: redFields.has(`row:${group}:${item.key}:${field.key}`) || missingRed.has(`row:${group}:${item.key}:${field.key}`) ? colors.error + "1A" : FIELD_BG, borderColor: redFields.has(`row:${group}:${item.key}:${field.key}`) || missingRed.has(`row:${group}:${item.key}:${field.key}`) ? colors.error : FIELD_BORDER, color: colors.foreground, writingDirection: field.mono ? "ltr" : undefined, textAlign: field.mono ? "left" : align }]} />)}{rowErrors[item.key] ? <Text style={{ color: colors.error, fontSize: 10, marginTop: 4 }}>{rowErrors[item.key]}</Text> : null}{index === rows.length - 1 ? <RipplePressable rippleColor={colors.primary + "22"} onPress={addRow} style={({ pressed }) => [styles.addRowButton, { borderColor: colors.primary + "66", opacity: pressed ? 0.7 : 1, marginTop: 8 }]}><MaterialIcons name="add" size={15} color={colors.primary} /><Text style={{ color: colors.primary, fontSize: 11.5, fontWeight: "800" }}>{group === "cliq" ? (language === "ar" ? "+ إضافة حساب CliQ آخر" : "+ Add another CliQ account") : (language === "ar" ? "+ إضافة حساب بنكي آخر" : "+ Add another bank account")}</Text></RipplePressable> : null}</View></View> : <View key={item.key} onLayout={(event) => { layoutY.current["row-" + item.key] = event.nativeEvent.layout.y; }} style={[styles.rowSubCard, { backgroundColor: colors.surfaceMuted + "66", borderColor: colors.border, marginTop: index === 0 ? 0 : 8 }]}>
        ...
        {group === "other" && index === rows.length - 1 ? <RipplePressable rippleColor={colors.primary + "22"} onPress={addRow} style={({ pressed }) => [styles.addRowButton, { borderColor: colors.primary + "66", opacity: pressed ? 0.7 : 1 }]}><MaterialIcons name="add" size={15} color={colors.primary} /><Text style={{ color: colors.primary, fontSize: 11.5, fontWeight: "800" }}>{language === "ar" ? "+ إضافة طريقة أخرى" : "+ Add another custom method"}</Text></RipplePressable> : null}
      </View>;
    });
  };
  ...
  const ownerChannelSection = (group: "cliq" | "bank" | "vault" | "other") => {
    ...
    return <View onLayout={(event) => { layoutY.current["sec-" + group] = event.nativeEvent.layout.y; }} style={[styles.channelCard, { backgroundColor: FIELD_BG, borderColor: FIELD_BORDER }]}>
  ...
  const teamOptions: TeamEntry[] = [
    { key: "owner", kind: "owner", icon: "account-balance", name: language === "ar" ? "الخزينة المركزية / المالك" : "Central treasury / Owner" },
    ...selectableMembers.map((member): TeamEntry => ({ key: "member:" + member.userId, kind: "member", userId: member.userId, icon: "person", name: member.displayName, phone: member.phone ?? "", userCode: member.userCode, roleLabel: memberRoleLabel(member.role, language) })),
    ...onbookStaff.map((entry): TeamEntry => ({ key: "onbook:" + entry.uid, kind: "onbook", uid: entry.uid, icon: "badge", name: entry.name, phone: entry.phone, role: entry.role, roleLabel: onbookRoleLabel(entry.role, language) })),
  ];
  const applyPersonChoice = (option: TeamEntry) => {
    setTeamPickerOpen(false);
    setTeamChoice(option);
    if (option.kind === "owner") {
      setUnifiedEntity("owner");
      setOpenChannels({ cliq: false, bank: false, cash: false, other: false });
      setEditingAccountId(null);
      return;
    }
    const entity: "staff" | "guard" = option.kind === "onbook" && option.role === "guard" ? "guard" : "staff";
    setUnifiedEntity(entity);
    setFloatDraft((draft) => ({ ...draft, entity, onbookUid: option.kind === "onbook" ? option.uid : "", uid: option.kind === "member" ? (option.userCode ?? "") : "", label: draft.label.trim() ? draft.label : option.name, phone: option.phone }));
    setOpenChannels({ cliq: false, bank: false, cash: false, other: false });
  };
  const pickPerson = (option: TeamEntry) => {
    setTeamPickerOpen(false);
    applyPersonChoice(option);
  };
  const quickAddTeamMember = async ({ name, phone, role, permissions }: { name: string; phone: string; role: AddUserRole; permissions: WorkspacePermissions }): Promise<string | null> => {
    ...
    if (byPhone) {
      if (byPhone.role === "owner") return language === "ar" ? "هذا الهاتف ملك المالك الأساسي — اختر عضوًا آخر." : "This phone belongs to the primary owner — pick a different member.";
      try {
    ...
    try {
      ...
      return language === "ar" ? "تمت الإضافة محليًا لكن تعذر إرسال الدعوة." : "Added locally but the invitation could not be sent.";
    }
    applyPersonChoice({ key: "onbook:" + newUid, kind: "onbook", uid: newUid, icon: "badge", name: name.trim(), phone: phone.trim(), role: role === "guard" ? "guard" : "staff", roleLabel: onbookRoleLabel(role === "guard" ? "guard" : "staff", language) });
    Alert.alert(language === "ar" ? "✓ تمت الإضافة والدعوة" : "✓ Added & invited", language === "ar" ? "أُضيف الموظف للفريق واختير افتراضيًا لنقطة التحصيل." : "The employee was added to the team and pre-selected for the collection point.");
    return null;
  };
...
<View style={[styles.custodyCard, { backgroundColor: colors.surface, borderColor: "#0EA5E9" + "60" }]}><View style={[styles.badge, { backgroundColor: "#0EA5E9" + "0D", borderColor: "#0EA5E9" + "38", flexDirection: row }]}><MaterialIcons name="info-outline" size={17} color="#0284C7" /><Text style={[styles.flex, { color: colors.muted, fontSize: 11.5, lineHeight: 18, fontWeight: "700", textAlign: align }]}>{language === "ar" ? "المبالغ المستلمة هنا تُسجل كذمة مالية/عهدة معلقة على الموظف لحين التوريد والتسوية مع المالك" : "Funds received here are recorded as a liability/float due from the employee until handed over and settled with the owner"}</Text></View>{floats.length ? <View style={styles.list}>{floats.map((account) => { const isGuard = account.entity === "guard"; const floatIcon = isGuard ? "security" : "badge"; const floatTint = isGuard ? "#8B5CF6" : "#0EA5E9"; const floatId = "float-parent-" + account.id; const expanded = expandedIds.has(floatId); const cashChannels = Array.isArray(account.channels) ? account.channels.filter((c) => c.kind === "cliq") : []; const bankChannels = Array.isArray(account.channels) ? account.channels.filter((c) => c.kind === "bank") : []; const keyCount = (account.cashActive === true ? 1 : 0) + cashChannels.filter((c) => c.isActive !== false).length + bankChannels.filter((c) => c.isActive !== false).length + (account.hasOther ? 1 : 0); return <View key={account.id} style={[styles.floatParent, { backgroundColor: colors.background, borderColor: colors.border }]}><Pressable accessibilityRole="button" accessibilityLabel={`${language === "ar" ? "توسيع" : "Expand"} ${account.label}`} onPress={() => toggleExpanded(floatId)} style={{ width: "100%" }}><View style={[styles.channelRowHead, { flexDirection: cardRow, direction: "ltr" }]}><View style={[styles.channelIcon, { backgroundColor: floatTint + "14", borderColor: floatTint + "35" }]}><MaterialIcons name={floatIcon} size={20} color={floatTint} /></View><View style={styles.flex}><View style={[styles.channelTitleRow, { flexDirection: row }]}><Text numberOfLines={1} style={{ color: colors.foreground, fontSize: 14, fontWeight: "900", textAlign: align }}>{account.label}</Text>{renderStatusPill(isGuard ? (language === "ar" ? "حارس" : "Guard") : (language === "ar" ? "موظف" : "Staff"), floatTint)}{renderStatusPill(language === "ar" ? `${keyCount} ${keyCount === 1 ? "قناة مسجلة" : "قنوات مسجلة"}` : `${keyCount} registered channel${keyCount === 1 ? "" : "s"}`, colors.muted)}{!isFloatComplete(account) ? renderStatusPill(language === "ar" ? "⚠️ بيانات غير مكتملة" : "⚠️ Incomplete data", colors.warning) : account.isActive !== false ? renderStatusPill(language === "ar" ? "✓ مفعّل وجاهز" : "✓ Active", colors.success, "check-circle") : renderStatusPill(language === "ar" ? "موقوف" : "Paused", colors.warning, "pause-circle")}{account.isDefault ? renderStatusPill(language === "ar" ? "افتراضي" : "Default", colors.success, "star") : null}</View><Text numberOfLines={1} style={{ color: account.isActive !== false ? colors.muted : colors.warning, fontSize: 10.5, fontWeight: "700", marginTop: 3, textAlign: align, fontFamily: MONO_FONT }}>{[account.memberName ? `${language === "ar" ? "الموظف" : "Employee"}: ${account.memberName}` : "", account.contactPhone ? `${language === "ar" ? "الهاتف" : "Phone"}: ${account.contactPhone}` : "", account.maxFloatLimit !== undefined ? `${language === "ar" ? "سقف العهدة" : "Float ceiling"}: ${account.maxFloatLimit}` : ""].filter(Boolean).join(" · ")}</Text></View><ChannelSwitch value={account.isActive !== false} onValueChange={(value) => toggleFloatActive(account, value)} isRTL={isRTL} color={account.isActive !== false ? colors.success : colors.warning} inactiveColor="#334155" accessibilityLabel={`${language === "ar" ? "تفعيل" : "Enable"} ${account.label}`} /><Pressable accessibilityRole="button" accessibilityLabel={language === "ar" ? "تعديل نقطة التحصيل" : "Edit collection point"} onPress={() => openFloatEdit(account)} style={({ pressed }) => [styles.iconButton, { borderColor: floatTint + "55", opacity: pressed ? 0.7 : 1 }]}><MaterialIcons name="edit" size={17} color={floatTint} /></Pressable><Pressable accessibilityRole="button" accessibilityLabel={language === "ar" ? "حذف نقطة التحصيل" : "Remove collection point"} onPress={() => removeFloat(account)} style={({ pressed }) => [styles.iconButton, { borderColor: colors.error + "55", opacity: pressed ? 0.7 : 1 }]}><MaterialIcons name="delete-outline" size={17} color={colors.error} /></Pressable><MaterialIcons name={expanded ? "keyboard-arrow-up" : "keyboard-arrow-down"} size={21} color={colors.muted} /></View></Pressable>{expanded ? <View style={styles.list}>{renderFloatCashBlock(account)}{renderFloatChannelGroup(account, "cliq")}{renderFloatChannelGroup(account, "bank")}{renderFloatOtherBlock(account)}</View> : null}</View>; })}</View> : null}</View>
    ...
    <Modal transparent visible={unifiedOpen} animationType="fade" onRequestClose={() => { setUnifiedOpen(false); setEditingAccountId(null); clearAuditHighlight(); }} statusBarTranslucent><View style={styles.backdrop}><Pressable style={StyleSheet.absoluteFill} onPress={() => { setUnifiedOpen(false); setEditingAccountId(null); clearAuditHighlight(); }} /><View style={[styles.sheet, styles.unifiedSheet, { backgroundColor: colors.surface, borderColor: colors.border }]}><View style={[styles.sheetHeader, { flexDirection: row }]}><Text style={[styles.flex, { color: colors.foreground, fontSize: 18, fontWeight: "900", textAlign: align }]}>{modalTitle}</Text><Pressable accessibilityLabel={language === "ar" ? "إغلاق" : "Close"} onPress={() => { setUnifiedOpen(false); setEditingAccountId(null); clearAuditHighlight(); }} style={[styles.close, { backgroundColor: colors.surfaceMuted }]}><MaterialIcons name="close" size={20} color={colors.muted} /></Pressable></View><View onLayout={(event) => { layoutY.current["sec-identity"] = event.nativeEvent.layout.y; }} style={[styles.identityCard, { backgroundColor: colors.surfaceMuted + "3A", borderColor: FIELD_BORDER, marginBottom: 10 }]}><Text style={[styles.accountLabel, { color: colors.foreground, textAlign: align, marginTop: 0 }]}>{language === "ar" ? "اختيار المسؤول عن نقطة التحصيل" : "Pick the responsible person"}</Text><Text style={{ color: colors.muted, fontSize: 10.5, lineHeight: 15, marginTop: 2, textAlign: align }}>{language === "ar" ? "كاش أو CliQ — كل تحصيل يُربط تلقائيًا برقم هاتف العضو وسِجله المالي بغض النظر عن حالة تطبيقه." : "Cash or CliQ — every collection is attached to the member's phone & ledger automatically, regardless of app status."}</Text><Pressable accessibilityRole="button" onPress={() => setTeamPickerOpen(true)} style={[styles.pickerTrigger, { backgroundColor: FIELD_BG, borderColor: FIELD_BORDER, flexDirection: row }]}>{teamChoice ? <MaterialIcons name={teamChoice.icon} size={18} color={teamChoice.kind === "owner" ? colors.primary : "#0EA5E9"} /> : <MaterialIcons name="expand-more" size={20} color={colors.muted} />}<View style={styles.flex}><Text style={{ color: teamChoice ? colors.foreground : colors.muted, fontSize: 13, fontWeight: "800", textAlign: align }}>{teamChoice ? teamChoice.name : (language === "ar" ? "اختر المسؤول..." : "Choose the person...")}</Text>{teamChoice && teamChoice.kind !== "owner" ? <Text style={{ color: colors.muted, fontSize: 10.5, marginTop: 1, textAlign: align, fontFamily: MONO_FONT }}>{teamChoice.phone ? teamChoice.phone : teamChoice.roleLabel}</Text> : null}</View><MaterialIcons name="keyboard-arrow-down" size={20} color={colors.muted} /></Pressable></View><ScrollView ref={sheetScrollRef} style={{ flex: 1 }} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled" contentContainerStyle={{ paddingBottom: 10 }}>
      ...
      {unifiedEntity === "owner" ? <View>
        ...
        <View style={[styles.identityCard, { backgroundColor: colors.surfaceMuted + "3A", borderColor: FIELD_BORDER }]}><Text style={[styles.accountLabel, { color: colors.foreground, textAlign: align, marginTop: 0 }]}>{language === "ar" ? "اسم نقطة التحصيل" : "Collection point name"}</Text><Text style={{ color: colors.muted, fontSize: 10.5, lineHeight: 15, marginTop: 2, textAlign: align }}>{language === "ar" ? "يميّز هذه النقطة في سجل العهد؛ يمكنك تعديله لإنشاء نقطة كاش أو CliQ ثانية لنفس العضو." : "Labels this point in the float ledger; edit it to create a second cash or CliQ point for the same member."}</Text><TextInput value={floatDraft.label} onChangeText={(text) => patchDraft({ label: text })} maxLength={120} onFocus={() => setFocusedField("label")} onBlur={() => setFocusedField(null)} placeholder={language === "ar" ? "مثال: كاش الحارس أحمد، كليك المحاسب" : "e.g. guard Ahmed's cash, accountant's CliQ"} placeholderTextColor={colors.muted} style={[styles.input, { backgroundColor: redFields.has("ident:label") || missingRed.has("ident:label") ? "#4C051933" : FIELD_BG, borderColor: focusedField === "label" ? FIELD_FOCUS : (redFields.has("ident:label") || missingRed.has("ident:label") ? "#F43F5E" : FIELD_BORDER), borderWidth: focusedField === "label" ? 2 : 1, color: colors.foreground, textAlign: align }]} /></View>
        <Text style={[styles.modalSectionTitle, { color: colors.foreground, textAlign: align }]}>{language === "ar" ? "تفعيل القنوات المتاحة لهذه النقطة" : "Enable this point's channels"}</Text><Text style={{ color: colors.muted, fontSize: 11, lineHeight: 16, marginTop: 2, textAlign: align }}>{language === "ar" ? "استلام نقدي، والحوالة عبر CliQ، وتحويلات بنكية — فعّل ما تحتاجه هذه النقطة." : "Cash in hand, CliQ transfer, and bank transfers — enable what this point needs."}</Text>
        ...
        <Text style={[styles.accountLabel, { color: colors.foreground, textAlign: align }]}>{language === "ar" ? "الحد الأقصى للنقدية (سقف الكاش)" : "Maximum cash (ceiling alert)"}</Text><TextInput accessibilityLabel={language === "ar" ? "الحد الأقصى المسموح للعهدة النقدية" : "Maximum allowed float ceiling"} value={floatDraft.ceiling} onChangeText={(text) => patchDraft({ ceiling: text })} keyboardType="decimal-pad" onFocus={() => setFocusedField("ceiling")} onBlur={() => setFocusedField(null)} placeholder={language === "ar" ? "مثال: 300 د.أ" : "Example: 300"} placeholderTextColor={colors.muted} style={[styles.input, { backgroundColor: redFields.has("ident:ceiling") || missingRed.has("ident:ceiling") ? colors.error + "1A" : FIELD_BG, borderColor: focusedField === "ceiling" ? FIELD_FOCUS : (redFields.has("ident:ceiling") || missingRed.has("ident:ceiling") ? colors.error : FIELD_BORDER), borderWidth: focusedField === "ceiling" ? 2 : 1, color: colors.foreground, writingDirection: "ltr", textAlign: "left" }]} /><Text style={{ color: colors.muted, fontSize: 10.5, marginTop: 5, textAlign: align }}>{language === "ar" ? "لتنبيه النظام عند تجاوز مبالغ الكاش المستلمة لدى هذا المستلم." : "System alerts when cash collected by this point exceeds the ceiling."}</Text>
      </View>}
    ...
    <Modal transparent visible={teamPickerOpen} animationType="fade" onRequestClose={() => setTeamPickerOpen(false)} statusBarTranslucent><View style={styles.backdrop}><Pressable style={StyleSheet.absoluteFill} onPress={() => setTeamPickerOpen(false)} /><View style={[styles.pickerSheet, { backgroundColor: colors.surface, borderColor: colors.border }]}><View style={[styles.sheetHeader, { flexDirection: row }]}><Text style={[styles.flex, { color: colors.foreground, fontSize: 16, fontWeight: "900", textAlign: align }]}>{language === "ar" ? "اختيار المسؤول عن نقطة التحصيل" : "Pick the responsible person"}</Text><Pressable accessibilityLabel={language === "ar" ? "إغلاق" : "Close"} onPress={() => setTeamPickerOpen(false)} style={[styles.close, { backgroundColor: colors.surfaceMuted }]}><MaterialIcons name="close" size={18} color={colors.muted} /></Pressable></View><ScrollView style={{ maxHeight: 360 }} showsVerticalScrollIndicator={false}>{teamOptions.map((option) => { const selected = teamChoice?.key === option.key; return <Pressable key={option.key} accessibilityRole="button" accessibilityState={{ selected }} onPress={() => pickPerson(option)} style={[styles.pickerRow, { backgroundColor: selected ? colors.primary + "1A" : FIELD_BG, borderColor: selected ? colors.primary : FIELD_BORDER, flexDirection: row }]}><View style={[styles.pickerRowIcon, { backgroundColor: (option.kind === "owner" ? colors.primary : "#0EA5E9") + "14", borderColor: (option.kind === "owner" ? colors.primary : "#0EA5E9") + "35" }]}><MaterialIcons name={option.icon} size={18} color={option.kind === "owner" ? colors.primary : "#0EA5E9"} /></View><View style={styles.flex}><Text style={{ color: colors.foreground, fontSize: 13.5, fontWeight: "800", textAlign: align }}>{option.name}</Text><Text style={{ color: colors.muted, fontSize: 10.5, marginTop: 1, textAlign: align }}>{option.kind === "owner" ? (language === "ar" ? "الخزينة المركزية — المالك" : "Central treasury — Owner") : (option.phone ? `${option.phone} (${option.roleLabel})` : option.roleLabel)}</Text></View>{selected ? <MaterialIcons name="check-circle" size={19} color={colors.primary} /> : null}</Pressable>; })}</ScrollView><Pressable accessibilityRole="button" accessibilityLabel={language === "ar" ? "إضافة عضو / موظف جديد للفريق" : "Add a new team member"} onPress={() => { setTeamPickerOpen(false); setAddUserOpen(true); }} style={({ pressed }) => [styles.quickAddStaff, { backgroundColor: colors.primary + "1A", borderColor: colors.primary, opacity: pressed ? 0.7 : 1, flexDirection: row, marginTop: 8 }]}><MaterialIcons name="person-add-alt-1" size={16} color={colors.primary} /><Text style={{ color: colors.primary, fontWeight: "900", fontSize: 12, textAlign: "center" }}>{language === "ar" ? "➕ إضافة عضو / موظف جديد للفريق" : "➕ Add a new team member"}</Text></Pressable></View></View></Modal>

    <AddUserModal visible={addUserOpen} language={language} isRTL={isRTL} colors={colors} onClose={() => setAddUserOpen(false)} onSubmit={quickAddTeamMember} />
...
const styles = StyleSheet.create({
  ...
  compactChannelCard: { borderWidth: 1, borderRadius: 13, padding: 10, gap: 10, alignItems: "flex-start" },
  compactChannelActions: { alignItems: "center", gap: 7, flexShrink: 0 },
  compactChannelAction: { width: 33, height: 33, borderRadius: 9, alignItems: "center", justifyContent: "center", borderWidth: 1 },
  compactInput: { minHeight: 34, borderWidth: 1, borderRadius: 9, paddingHorizontal: 10, paddingVertical: 6, fontSize: 12, marginTop: 6 },
...
floatParent: { borderWidth: 1, borderRadius: 16, padding: 12, gap: 10 },
  ...
  pickerTrigger: { minHeight: 46, borderWidth: 1, borderRadius: 13, paddingHorizontal: 12, alignItems: "center", gap: 9, marginTop: 7 },
  pickerSheet: { width: "100%", maxWidth: 460, borderWidth: 1, borderRadius: 22, padding: 14, gap: 4, maxHeight: "82%" },
  pickerRow: { minHeight: 54, borderWidth: 1, borderRadius: 12, paddingHorizontal: 10, paddingVertical: 8, alignItems: "center", gap: 8 },
  quickAddStaff: { minHeight: 46, borderWidth: 1, borderStyle: "dashed", borderRadius: 13, paddingHorizontal: 11, alignItems: "center", justifyContent: "center", gap: 7 },
  pickerRowIcon: { width: 34, height: 34, borderRadius: 10, alignItems: "center", justifyContent: "center" },
});
