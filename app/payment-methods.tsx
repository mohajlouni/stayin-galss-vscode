...
type FloatDraft = { id?: string; entity: "staff" | "guard"; label: string; uid: string; phone: string; hasOther: boolean; otherNote: string; ceiling: string; isActive: boolean; isDefault: boolean };
...
const emptyFloatDraft = (entity: "staff" | "guard"): FloatDraft => ({ entity, label: "", uid: "", phone: "", hasOther: false, otherNote: "", ceiling: "", isActive: false, isDefault: false });
...
export default function PaymentMethodsScreen() {
  ...
  const openFloatEdit = (account: StaffFloatAccount) => {
    ...
    setFloatDraft({ id: account.id, entity, label: account.label, uid: selectableMembers.find((member) => member.userId === account.memberUserId)?.userCode ?? "", phone: account.contactPhone ?? "", hasOther: account.hasOther === true, otherNote: ((account.bankDetails ?? "").split("\n").find((line) => line.startsWith("أخرى: ")) ?? "").replace(/^أخرى: /, "").trim(), ceiling: account.maxFloatLimit !== undefined ? String(account.maxFloatLimit) : "", isActive: account.cashActive === true, isDefault: account.isDefault === true });
