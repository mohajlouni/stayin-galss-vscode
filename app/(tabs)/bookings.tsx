    ...
    <ScreenContainer>
      <View style={styles.screen}>
        <View style={styles.headerSection}>
          ...
          {query.trim() ? <View style={[styles.searchSummary, { backgroundColor: colors.surfaceMuted, borderColor: colors.border, flexDirection: row }]}><MaterialIcons name="filter-list" size={13} color="#F59E0B" /><Text numberOfLines={1} style={{ flex: 1, color: colors.foreground, fontSize: 10.5, fontWeight: "800", textAlign: align }}>{language === "ar" ? `عرض (${filtered.length}) نتائج مطابقة لـ "${query.trim()}"` : `Showing (${filtered.length}) matches for "${query.trim()}"`}</Text><Pressable accessibilityRole="button" accessibilityLabel={language === "ar" ? "مسح البحث" : "Clear search"} onPress={() => setQuery("")} hitSlop={8}><MaterialIcons name="close" size={15} color={colors.muted} /></Pressable></View> : null}
        </View>
...
const styles = StyleSheet.create({
  ...
  searchSummary: { minHeight: 34, borderRadius: 10, paddingHorizontal: 10, alignItems: "center", gap: 6, marginBottom: 8, borderWidth: 1 },
  ...
  waitlistPriority: { borderRadius: 18, marginTop: 8 },
  waitlistPriorityContent: { padding: 10, minHeight: 58 },
  ...
  waitlistSummary: { minHeight: 58, borderRadius: 16, marginTop: 8 },
  waitlistSummaryContent: { flexDirection: "row", alignItems: "center", gap: 7, paddingHorizontal: 9, paddingVertical: 10, minHeight: 58 },
