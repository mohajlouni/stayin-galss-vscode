...
export function BookingProvider({ children }: { children: React.ReactNode }) {
  ...
  const value = useMemo<BookingContextValue>(() => ({
    ...
    completeMaintenanceTaskWithExpense: async (id, input: { performedByName: string; performedByRole: MaintenancePerformerRole; actualCost?: number; completionNotes?: string; postExpense: boolean; scheduleNext?: boolean; funding?: { entity: "owner" | "staff"; channel?: ExpenseFundingChannel; sourceId?: string; sourceLabel?: string; mode?: "float" | "reimbursement" } }) => {
      ...
      const completedTask: MaintenanceTask = { ...task, status: "completed", lastCompletedDate: completedAt.slice(0, 10), nextDueDate: nextMaintenanceDueDate({ ...task, lastCompletedDate: completedAt.slice(0, 10) }), completedAt, completedByName: actorName, performedById: user?.id, performedByName: input.performedByName.trim(), performedByRole: input.performedByRole, actualCost: cost, paymentSource: derivedSource, expenseFundingEntity: shouldPost ? funding?.entity : undefined, expenseFundingChannel: shouldPost && funding?.entity === "owner" ? funding?.channel : undefined, expenseFundingSourceLabel: shouldPost ? (funding?.sourceLabel ?? (funding?.entity === "staff" ? (data.settings.paymentRouting?.staffFloats ?? []).find((account) => account.id === funding?.sourceId)?.label : undefined)) : undefined, expenseFundingMode: shouldPost && funding?.entity === "staff" ? funding?.mode : undefined, completionNotes: input.completionNotes?.trim() || undefined, expenseId: !shouldPost ? task.expenseId : undefined };
