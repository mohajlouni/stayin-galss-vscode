...
export default function ChaletManagementScreen() {
  ...
  return <ScreenContainer edges={["top", "bottom", "left", "right"]}>
    <FlatList
      ...
      ListHeaderComponent={<>
        <SubScreenHeader title={language === "ar" ? "إدارة الوحدات / العقارات" : "Property management"} fallbackHref="/properties-hub" />
      ...
      ListEmptyComponent={chalets.length ? (
        <View style={[styles.empty, { backgroundColor: colors.surface, borderColor: colors.border }]}><MaterialIcons name="search-off" size={28} color={colors.muted} /><Text style={{ color: colors.foreground, fontWeight: "800", marginTop: 9 }}>{language === "ar" ? "لا توجد وحدات مطابقة" : "No matching properties"}</Text></View>
      ) : (
        <View style={[styles.empty, { backgroundColor: colors.surface, borderColor: colors.border }]}>
          <MaterialIcons name="home-work" size={28} color={colors.primary} />
          <Text style={{ color: colors.foreground, fontWeight: "800", marginTop: 9 }}>{language === "ar" ? "لا توجد وحدات مضافة بعد لهذه المنشأة" : "No units have been added to this property yet"}</Text>
          <Pressable onPress={() => openProfile()} style={({ pressed }) => [styles.emptyCta, { backgroundColor: colors.primary, flexDirection: row, opacity: pressed ? 0.78 : 1 }]}><MaterialIcons name="add" size={18} color={colors.background} /><Text style={{ color: colors.background, fontWeight: "900", fontSize: 13 }}>{language === "ar" ? "إضافة وحدة جديدة +" : "Add new unit +"}</Text></Pressable>
        </View>
      )}
