...
export default function BookingDetail() {
  ...
  const { bookings, waitlist, chalets, settings, auditLog, addDepositRefund, addPayment, updatePayment, voidPayment, cancelBooking, markBookingCheckedIn, completeBookingStay, correctBookingStay } = useBookings();
  ...
  return (
    <ScreenContainer>
      ...
      <CheckOutConfirmationSheet booking={booking} visible={checkOutSheetOpen} saving={operationalSaving} colors={colors} currency={settings.currency} language={language} isRTL={isRTL} onClose={() => setCheckOutSheetOpen(false)} onConfirm={(confirmation) => void recordGuestCheckout(confirmation)} />
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false} removeClippedSubviews={false} keyboardShouldPersistTaps="handled">
        ...
        <View style={[styles.quickActions, { flexDirection: row }]}><QuickAction label={language === "ar" ? "واتساب" : "WhatsApp"} icon="chat" colors={colors} accentColor={colors.primary} onPress={openWhatsAppComposer} /><QuickAction label={language === "ar" ? "اتصال" : "Call"} icon="phone" colors={colors} accentColor={colors.primary} onPress={callGuest} /><QuickAction label={language === "ar" ? "الإيصال" : "Receipt"} icon="receipt-long" colors={colors} accentColor={colors.primary} onPress={() => setReceiptOpen(true)} /></View>
...
function QuickAction({ label, icon, colors, onPress, success = false, accentColor }: { label: string; icon: "receipt-long" | "edit" | "phone" | "chat"; colors: ReturnType<typeof useColors>; onPress: () => void; success?: boolean; accentColor?: string }) {
